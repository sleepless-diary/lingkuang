...
- **逐元素的 WAAPI 原语**（不走 CSS 类，因为元素是**被 diff 出来的**、不是"重放一次入场"）：
  ...
  ⚠️ **整批换（换档）不许并行**：那是"两把尺子叠在一起"（用户 2026-09-26 实测）；判据 = 换掉的
  比例 ≥ `WHOLE_SWAP_RATIO(0.5)` ⇒ 退场批 `{dur: 100, step: 0}`、入场批 `start: 100`，
