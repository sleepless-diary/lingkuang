...
- ⭐ **左树的行级动效（2026-09-13，用户：「新建实体和节点时不是硬切换，而是从左侧滑入…其下的所有
  ...
  - ⭐ **收起时的退场也在这套原语上**（用户：「设定文件夹收起时无动画」→「文件出场动画改成入场的反向
    ...
    `leave = (ghosts) => rowsLeaveAndRemove(ghosts, EXIT)`，**`EXIT = { dy: 8, dur: 180, step: 14, maxDelay: 120 }`**
    （2026-09-14 第二轮提速，用户：「文件收起的动画快一点，现在有一点停滞感」；帧条那边用同一档参数）——
    **与 `rowsDropIn` 同序**（自上而下 0/14/28…，**不要 `reverse()`**：入场是从上往下一行行出来，
    退场倒着播等于波浪反向，与入场对不上）、方向相反（往上 8px）、曲线倒过来（慢→快）。
    `rowsLeave` / `rowsDropIn` 的**错峰上限 `MAX_STAGGER`**：不限量时 44 行会让第一行等 **946ms**
    才动，看着就是"卡住不动然后整片消失"。
- ⭐ **"里面的文件先消失，下面的文件夹再移上来"**（用户 2026-09-14：「应该是文件先消失，下面的文件夹再
  移上来，现在反了」）：`collapse()` 里 `flipDelayMs = Math.round(rowsLeaveTotal(g.length, EXIT) * FLIP_OVERLAP)`，
  **`FLIP_OVERLAP = 0.55`**（等满整枝就是用户第二天说的"停一拍"的停滞感），`renderList()` 把它传给
  **`flipRows(..., { delay })`**。
  ⚠️ `flipRows` 给了 `delay` 就**必须** `fill: 'both'` —— 延迟期间要冻在旧位置上；`fill:'none'` 的话那一行
  会先瞬移到新位置、等延迟过完再跳回旧位置演一遍。实测收「主线」那一枝：3 个幽灵 delay 0/22/44 + dur 260
  ⇒ 退场总时长 **304ms**，下面 45 行 FLIP 的 delay 全是 **304**、`fill:"both"`。
- ⭐ **补位的行要"越高的越先移"**（用户 2026-09-14：「文件夹收起后其下文件上移**错分方向反了**，应该是
  ...
  不留空洞），`src/ui/codex.ts` 用 `FLIP_STEP = 14` / `FLIP_MAX = 120`。没有它就只有一个统一 `delay`，
  整片一起上移（看着像"被拽上去"，不是"一行行抽上去"）。
