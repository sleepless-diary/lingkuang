...
| `src/ui/agent-mode.ts` | 助手的**两种模式**（第 4 片，用户 2026-09-26：「我想让灵框平常是正常的聊天及工作，但是少数情况下有 agent 工作能力」）：`export type AgentMode = 'chat' \| 'agent'`、`MODES`、`MODE_LABEL`（聊天 / Agent）、`MODE_HINT`、`modePrompt(mode)`（chat 返回「【你现在是「聊天」模式】你只能聊天：读不到动作、也改不了灵框里的任何数据…不要说「我已经改好了」…也不要输出 JSON 指令」，agent 返回空串）。⭐ **模式不落盘、不进设置**：每次都从 chat 开始 —— 「少数情况」不该变成"忘了切回来"。落地方式 = `src/ui/agent.ts` 按模式决定**注不注入协议**（`toolsPrompt()`/`permissionPrompt()`）、**解不解析动作**（`parseToolCall` / `isCallMsg`），面板上则换 `data-mode` / `.is-agent` / 分段按钮 `.is-on` |
