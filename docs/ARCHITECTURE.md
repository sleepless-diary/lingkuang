...
| `src/ui/ai.ts` | **统一 AI 引擎**（联想 / 工作台 / 角色扮演 / 酒馆 / 助手共用同一条出口）：`aiChat(messages, opts?: { model?; temperature?; numPredict? })` 读 `loadSettings()` 决定走哪条线 —— `aiMode === 'api'` 时 POST `{baseUrl}/chat/completions`（`Bearer apiKey`、`max_tokens`），否则 POST `{baseUrl}/api/chat`（Ollama，`options: { temperature, num_predict }`）；模型 = `opts.model \|\| cfg.model`。⚠️ 角色扮演（`roleplay.ts:52`）与酒馆（`tavern.ts:72`）**写死了 `qwen3:14b`**、工作台另有 `MODEL` 常量 ⇒ 设置里换了模型**不影响**这三处（见 `docs/BUGS.md` 第五十轮 ①）。qwen3 的内容可能全在 `thinking` ⇒ 回复取 `content \|\| thinking` 兜底 |
