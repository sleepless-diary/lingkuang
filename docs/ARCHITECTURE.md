...
| `src/ui/codex.ts` | **设定库 = 工作台**（合并方案 A 第 3 步 + 2026-09-13 的形态合并与左栏重做）：左列**两个控件**、中栏档案字段、右栏正文编辑器、右边缘演变帧条，外加**外部改动提示条**（`src/ui/vault-notice.ts`，见那一行）。节点中栏用 `src/ui/props-panel.ts`（与实体共用同一份）、实体用 `src/ui/fields.ts`、正文用 `src/ui/doc-editor.ts`。⭐ **左栏只有「筛什么」+「怎么摆」两个控件**（用户 2026-09-13：「时间线节点和实体这两个按钮，列表和文件夹树的功能有点混乱，能不能重新设计一下」→ 撤掉两个页签）：① 筛选 pills `#cx-chips [data-cx-chip]` = `全部 N` / 各实体类型 `角色 1` / `时间线节点 M`（计数**按当前搜索词**算），② 视图按钮 `#cx-view`（文案 = 当前形态 `视图 列表` / `视图 文件夹`，`title` 说点下去变成什么）。`mode`（编辑实体还是节点）由**点中的那一行**决定，不再是全局页签；两条路都走 `switchTarget()`。⭐ **两种形态**（`listView: 'list' \| 'tree'`，默认形态在「设置 → 设定库（工作台）」的 `Settings.workbenchView`，localStorage `lingkuang-settings`）：`'list'` = **一列平铺**的条目（按类别分组：`设定` / `时间线节点` 两个 `.ed-tgroup` 小标题，只有 `全部` 时显示），**不分世界/时间线层级**；`'tree'` = `renderTreeList()` 一棵**文件夹树**，跟硬盘目录一一对应：世界 → 时间线 → 种类 → 节点 ／ 世界 → `_设定` → 类型 → 实体（`data-act` = world/tl/tkind/wset/etype/node/entity）。两种形态共用同一套行渲染（`.ed-tnode` + `treeRow()`）与同一份点击处理（`bindTreeClicks()`），列表行多一个 `.ed-tflat`（不缩进）；实体行仍带 **`data-cx-id`**（旧版是 `<button data-cx-id>`，十几个老套件按它找行）。切形态**不整块重建**（只换左列 + 按钮文案，保住滚动位置与 tiptap）。⚠️ `treeRow()` 用 `setAttribute('data-' + k)` 而不是 `dataset[k]`：`cx-id` 这种带连字符的键走 dataset 会抛 `SyntaxError: 'cx-id' is not a valid property name`。⚠️ 两类"空"**待遇不同，别顺手统一**：节点**种类**只列真有节点的（= 硬盘上真有的目录，`[...used.keys()]`）—— 改成「`store.data.formats` 的全部种类」会被用户报回来（「我指的是角色，地点，物品等文件夹同时存在于主线与设定文件夹下，是bug」；`main.js` 的 `DEFAULT_FORMATS` 曾与实体类型 `BUILTIN_ENTITY_TYPES` 重名，现在只留 `事件`/`战斗`）；实体**类型**列**全部**（含一个实体都没有的，空的 `is-empty` 置灰 + 展开一句「这个类型还没有实体」）。`_设定` 是**世界下面的一层**（与时间线同级）⇒ 缩进跟 `.ed-ttl` 一样 `padding-left: 18px`。⚠️ 换条目必须走 `switchTarget()`（先 flush 再改选择）。⚠️ `normalizeEntitySelection()` 对着**全部实体**归一（不是筛过的那批）：筛选只决定"看什么"，切 pills 不该把正在编辑的实体清空。⭐ **同类别内换条目走「就地换内容」**（`swapBody()`，2026-09-13）：保住骨架，只换「名字/类型 + 字段行 + 正文」，内容区（`#cx-body`）播一次 `.lk-swap-in` 轻淡入 —— 整块 `render()` 会把左列错峰重播一遍、**把滚动位置打回顶部**（`#cx-root` 就是滚动容器）、还把 tiptap 销毁重建（用户描述为「点击实体会刷新界面」）。**换类别**（实体 ↔ 节点，骨架形状不同）或换世界/条目删空才整块重建。⭐ 编辑器的写回目标是**读时取值**的模块级 `docTarget`（编辑器跨条目复用），安全性由 `switchTarget` 的顺序保证：先 flush（旧目标）再改 `docTarget`。⚠️ store 订阅**按 `bodySignature()` 决定要不要重建**中/右栏 —— 无条件 `render()` 会 dispose 掉 tiptap，而「自动落盘 → vault 回扫」每次编辑后约 360ms 就会走一趟订阅，实测每换一次 DOM 就有丢击键/焦点/IME 的风险（第十八轮）。⚠️ 面板**高度预算**：`#cx-root` 是 `height:100%` + `overflow:auto`，内容比窗口高 1px 就长滚动条（实测只差 3px）⇒ `#cx-msg`/`#cx-hint` 没内容时 `display:none`（守卫：`entity-evolution.cjs` ★0d 外壳开销 ≤130px） |
