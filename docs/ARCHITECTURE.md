...
| 路径 | 职责 |
|---|---|
| `main.js` | Electron 主进程。IPC：`data:load/save`（世界观）、`settings`、`lib`（词库）、`ai:associate`（联想）、`ai:classify`（词分类）、`aiChat()`（双模式 LLM 调用；**2026-09-26 起**可随调用带一份白名单配置 —— `aiCfgOverride(raw)` 只收 `mode`/`baseUrl`/`model`/`apiKey`，给 `ai:classify` 用：供应商档案住在渲染进程，不带就会固定打 `localhost:11434`；连不上时抛人话 `连不上 <端点>：<原因>`）、`vault:*`（Obsidian 文稿：`scan`/`write`/`write-entity`/`delete`/`delete-entity`/`delete-timeline`/`delete-world`/`trash-list`/`trash-restore`/`trash-purge`）、`formats:load/save`（结构体格式）、`backup:list/create/restore/export/import`（备份管理）、`app:flush-sync`（退出前同步落盘）、`agent:load/save`（灵框助手的**对话与记忆** → `%APPDATA%\lingkuang\agent\chat.json`（裸数组，`AGENT_CHAT_MAX = 200` 截尾）+ `memory.json`；测试时落 `LINGKUANG_TEST_DATA` 同级 `agent/`） |
...
| `src/ui/assoc.ts` | 词义联想**无限画布**（力导向 + 单线聚焦 + 视窗平移/缩放 + 拖节点贴边自动推视窗 + 手动摆过的节点钉住，钉住上限 `PIN_YIELD = 420`）；拖拽中只免"手里那一格"、线的另一头照常受力（＝线上的拉力）；没有世界边界（`HOME_W/HOME_H` 只是初始落点区与 SVG 作图区），框外连线靠 `.assoc__lines { overflow: visible }`；宿主高度由 `src/ui/inspire.ts` 的 `fitAssocHeight()` 让开 sticky 工具条，滚动容器用 `scrollParent()` 现找。「导出暂存词」把**当前在用**的供应商随调用带给主进程（`lingkuangAPI.classifyWords(staged, { mode, baseUrl, model, apiKey })` —— 归类跑在 `main.js` 的 `aiChat`，它只认老格式 `settings.json.ai`） |
...
| `src/ui/ai-workbench.ts` / `roleplay.ts` / `tavern.ts` | AI 工作台 / 角色扮演 / 酒馆剧情推演。**2026-09-26（AI 输出片）起**：三处的对话气泡都改走 `chat-live.ts`（流式）+ `md.ts`（markdown），气泡行内样式抽成各自的 `bubbleCss(who)`、容器挂 `lk-md` 类；对话类请求**不再带 `numPredict`**（原来角色扮演 400 / 酒馆 500 / 工作台 500 —— 那就是长回答被切半句的原因） | **2026-09-26（第 4 片 2C）起**：主会话（`role:'main'`）的发送**不再自己拼系统提示、也不再直接调 `aiChatStream`**，而是交给助手的 `runTurn()`（宿主 = 这一页：气泡画进 `#ai-log`、历史走 `pushMsg`、卡片由 `renderLog()` 末尾的 `cardsHtml()` 画出、提示写底部那行）；头上多一个「模式 聊天 / Agent」分段（`#lk-ai-mode-chat` / `#lk-ai-mode-agent`，状态读 `getAgentMode()`、点击走 `setAgentMode()`），并监听 `lingkuang-agent-mode` / `lingkuang-agent-cards` 让另一边的切换与卡片状态当场跟上（换工具重挂前用模块级 `agentWatch.off()` 摘旧监听）。非主会话（角色 / 视角 / 主控）**照旧**只演：`AI_SESSION_FRAME` + 人设 + 工作区现状，走 `aiChatStream`，没有动作协议。 **2026-09-26（思考片）起**：三处都接了 `onReasoning` —— 非主会话（工作台）与角色扮演、酒馆都会在气泡**上面**挂一块折叠的「思考」（`chat-live.ts` 的 `liveThink()`：生成中摊开、落定折叠、模型不吐思考就整块摘掉）。
| `src/ui/ai.ts` | **统一 AI 引擎**（联想 / 工作台 / 角色扮演 / 酒馆 / 助手共用同一条出口）：`aiChat(messages, opts?: { model?; temperature?; numPredict? })` 读 `activeProviderProfile()`（`settings.ts` → `ai-providers.ts`）决定走哪条线 —— 那一家的 `kind === 'openai'` 时 POST `{baseUrl}/chat/completions`（`Bearer apiKey`、`max_tokens`），否则 POST `{baseUrl}/api/chat`（Ollama，`options: { temperature, num_predict }`）；模型 = `opts.model \|\| p.model`，缺端点/Key/模型时由 `providerProblem(p)` 先抛人话。**2026-09-26（供应商片）起**：角色扮演（`src/ui/roleplay.ts`）、酒馆（`src/ui/tavern.ts`）、工作台（`src/ui/ai-workbench.ts`）**不再写死 `qwen3:14b`**，全部跟随当前供应商选中的模型（上一版「设置里换模型不影响这三处」那个坑已修）。qwen3 的内容可能全在 `thinking` ⇒ 回复取 `content \|\| thinking` 兜底。**2026-09-26（AI 输出片）起**：① 新增 **`aiChatStream(messages, opts?: { model?; temperature?; numPredict?; onDelta? })`**（请求体 `stream: true`，逐行解析 —— OpenAI 线 SSE `data: {…}` / `data: [DONE]`、Ollama 线 NDJSON 一行一个对象，两条线共用一个 `eat(obj)` 分发，所以**端点不理会 stream、直接回一整包 JSON 也吃得下**），`aiChat()` 退化成它的薄封装（不传 `onDelta`）；② **上限只在调用方显式给了 `numPredict` 时才发**（OpenAI 线不发 `max_tokens` 键、Ollama 线发 `num_predict: -1`）—— 上一版写死的 `?? 600`（助手另传 900）就是把长回答切在半句的元凶；③ `finish_reason` / `done_reason === 'length'` ⇒ `AiReply.truncated = true`（助手据此在底部如实说「被截断了」）。四个聊天面的流式都靠 `chat-live.ts` 的活气泡 | **2026-09-26（思考片）起**：思考（用户：「看不到他的思考诶，怎么办」）—— 新增 `AiOpts.onReasoning` / `AiReply.reasoning` / `ChatMsg.reasoning` 三个口子；字段名各家不一 ⇒ `firstStr()` 一次认 `delta.reasoning_content`/`delta.reasoning`/顶层 `reasoning_content|reasoning`（Ollama 线：`message.thinking|reasoning_content|reasoning`/顶层 `thinking`），`reasoning` **只在真有正文时回传**（content 为空时 thinking 就是答案本身，别重复画两遍）。⚠️ **消息上行一律过 `wire(messages)`**（只留 `{role, content}`）—— 历史里新挂的 `reasoning` 不能跟着 `messages` 进请求体，严格端点会 400。
| `src/ui/md.ts` | **极简 markdown → HTML**（用户 2026-09-26：「ai 的回答没被渲染，如 \`**文字**\` 这种」）：`mdToHtml(src: string): string`。分寸 = **先整段转义 `& < >` 再套标记**（不是先转标记再转义）⇒ 模型输出里的 HTML 永远是文字，不会被当标签执行。行内：行内码 / `**strong**` / `*em*` / `~~del~~` / 只认 `http(s)` 的 `[t](url)`；块级：围栏代码块（**含未闭合的** —— 流式过程中必然出现半个围栏）、`#`~`######`（封顶 h3）、`---`、`>` 引用、`-*+` 与 `1.` 列表；其余行按段聚合、**单换行 → `<br>`**。⚠️ **块与块之间不插换行**：老面板的气泡是行内 `white-space:pre-wrap`，插换行会多出一堆空行。消费者都在气泡容器上挂 `lk-md` 类拿样式（`src/style.css`） |
| `src/ui/chat-live.ts` | **「活气泡」= 把流式增量画进一个气泡**（用户 2026-09-26：「我想要流式输出」）：`liveBubble(parent: HTMLElement, o: LiveBubbleOpts): LiveBubble`，三层结构 wrap > inner > body（`wrapClass` / `innerClass` / `bodyClass` / `bodyStyle` / `scroll` 都可指定，老面板没有类名、靠 `style.cssText` 排版），句柄 `{ root, push(delta), placeholder(text), finish(text?), remove(), text() }`。`push()` 里 `body.innerHTML = mdToHtml(acc) + '<span class="lk-md__caret">'`（光标），`finish()` 摘 `.is-live`、去掉光标、再画一次终稿；`placeholder()` 画 `.lk-md__wait`（助手在 agent 模式吐动作 JSON 时改说「正在整理成动作…」）。⚠️ **`push()` 同步落 DOM，不用 rAF / 定时器节流** —— 测试实例 `showInactive`（窗口不可见）时 rAF 不跑、定时器被节流到 ~1s，那样写出来的流式在自动化里根本看不见（本项目已栽过两次） | **2026-09-26（思考片）起**：同一支里还有「思考」块 —— `liveThink(parent, { label?, scroll?, open? }): LiveThink`（`details.lk-think` 摊开 + `.is-live`，边想边长；`finish()` 折叠；**空思考整块摘掉**）与 `thinkBlockHtml(text)`（历史里那份，同一种 DOM 形状、默认折叠）。⚠️ 调用方要**先挂思考块再挂气泡**：DOM 追加顺序就是屏幕上下的顺序（反过来写，流式时思考在正文下面、落定重画后又跳回上面）。样式在 `src/style.css` 的 `.lk-think`。⚠️ **生成中**那一块是 `#lk-agent-msgs`（`display:flex; flex-direction:column; overflow-y:auto`）的**直接**子元素，而它自带 `overflow:hidden`（体内叠 `overflow-y:auto`）⇒ flex 项的自动最小尺寸退化成 0，列表一长就会被压到 **2px**、标题与正文全被自己裁掉（用户 2026-09-26 报「只能显示思考中、看不到思考过程」）⇒ `.lk-think` 必须 `flex: 0 0 auto`（`BUGS.md` 第六十轮；量这个坑必须在**生成中**量，落定后 `renderMsgs()` 已把它挪进消息内部、不再是弹性项）。
| `src/ui/ai-providers.ts` | AI **供应商档案**（用户 2026-09-26：「做成可选供应商和自定义供应商的版本吧」；照 DSH `dsh-client-ui-settings-models` 的 `ProviderEditor` / `CustomProviderCard` 三条分寸：**预设只产候选**、**自定义是「创建」不是「编辑」**、**失败点名是哪个字段**）：`ProviderProfile = { id, name, preset, kind: 'ollama' | 'openai', baseUrl, apiKey, model }`；`PRESETS`（本地 Ollama / DeepSeek / OpenAI / 硅基流动 / 自定义，顺序 = 面板菜单顺序）+ `providerFromPreset(id)` / `customProvider(id)` / `defaultProviders()`（= 一条本地 Ollama）；`sanitizeProvider(raw)` 洗落盘值；`activeOf({providers, activeProvider})` 取「当前在用」（指针丢了就退回第一条）；`providerSummary(p)`（`名字 · 模型`，助手 chip 与状态行用）；`providerProblem(p)` 缺什么就说清哪一项（AI 调用前先过它）；`probeCfgOf(p)` 折成 `ai-models` 的 `ProbeCfg`。⚠️ **本模块不 import `settings.ts`**（那边要 import 这里做迁移，反向 import 会成环 —— `activeOf()` 收的是结构类型）；「当前在用」的取用口是 `settings.ts` 的 `activeProviderProfile()` |
| `src/ui/ai-models.ts` | 设置面板「模型选择」的**数据层**（用户 2026-09-26：「设置里面的 ai 选择能不能改成像 dsh 里面的模型选择一样」；照 DSH `dsh-client-ui-settings-models` 的 `ModelListEditor` 那套分寸：**探测只产候选，用哪个由人点下去**，端点问不出来也不是死路）：`probeModels(cfg: ProbeCfg)` **永远 resolve**（失败当结果返回、不 throw）⇒ `{ ok, error, catalog }`。`aiMode === 'api'` 问 `{baseUrl}/models`（带 `Bearer apiKey`，解析 `data[].id` + `owned_by`）；否则问 `{baseUrl}/api/tags`（解析 `models[].name` + `details.parameter_size`/`quantization_level` + `size` 拼成小字），按 id 排序。`TIMEOUT_MS = 5000`（AbortController）。清单按 `cacheKey() = aiMode + '\|' + 去掉尾部斜杠的 baseUrl` 缓存进 localStorage **`lingkuang-model-cache`**（`readCatalog()` / `writeCatalog()`；`probeModels` 失败**不覆盖缓存**）⇒ 端点没开也还看得见上次那份。失败原因是人话（`端点返回 http 401（API Key 不对？）` / `连不上端点：…` / `端点 5 秒没响应` / `端点没报出任何模型`），直接画在设置面板那行小字上。**2026-09-26 供应商片起**：调用方用 `probeCfgOf(p)` 把某一家供应商折成 `ProbeCfg`（`kind → ollama/api`）—— 清单是**每家自己的**（缓存键仍含端点） |
| `src/ui/agent.ts` | **灵框助手**（内嵌 agent 第 1~3 片，用户 2026-09-15：「ai 真的工作，类 agent…有一个自己的对话框…全局快捷键」）：`openAgentPanel(store)` / `closeAgentPanel()` / `isAgentPanelOpen()` / `toggleAgentPanel(store)`。**右侧停靠**（`position:fixed; right:0; width:380px; z-index:1800`，**不接管主区** —— 聊天时照样能编稿子）；`Ctrl+K` / 左栏「助手」按钮 / Esc 三种开合，开关都广播 `lingkuang-panel` 让按钮同步高亮。它也是 **`Tool.panel = true` 的面板型工具**（与 `settings-panel.ts` 同一条路，但 `registry.ts` 只有**一格** `disposePanel` ⇒ 两个面板不能同时开）。发请求 = `agentAsk([{role:'system',content: [SYS_HEAD, modePrompt(mode), ...(tools ? [permissionPrompt(), toolsPrompt(m)] : []), memoryPrompt(), '【工作区现状】' + buildContext(store)].filter(Boolean).join('\n\n')}, ...history.slice(-HISTORY_SEND)])` —— 记忆与权限**都拼进系统提示**（不塞历史尾部：历史会被 `slice(-HISTORY_SEND)` 截掉）；**报错不进历史**，成功才 push + 落盘（2026-09-26 起 push 一律走 `pushHistory()` → `ai-sessions.ts` 的 `pushMsg`：**历史归主会话**，助手不再有自己的 `chat.json`；系统提示改由 `agentSystemPrompt(store, m = mode, tools = true)` 拼（第三个参数目前只有 AI 页主会话在用：阶段 2C 之前它还没有提议卡片 UI，先关掉协议），**AI 页的主会话用同一个函数**）。**2026-09-26（第 4 片）起**：面板头是「模式」分段（`#lk-agent-mode-chat` / `#lk-agent-mode-agent`，模块级 `mode: AgentMode` + `openEl.dataset.mode` + `.is-agent`），**默认 chat 且不落盘**（关掉面板再开 = 回到聊天，授权不残留）—— **2026-09-26 当天第二次改口后**（`BUGS.md` 第五十六轮）：两个模式**都注入协议与权限段**、**都解析动作**（`isCallMsg()` 不再按 mode 短路），差别只在自主程度 —— `CHAT_ROUNDS = 1`（一次提问只做一个动作）／`AGENT_ROUNDS = 8`（可以连着走多步 + 批量）；Agent 模式下权限是**两个正交旋钮**（`#lk-agent-scope` 范围 × `#lk-agent-ask` 询问，面板根元素带 `data-gate`），另有「长期记忆」区（加减改 + 从对话总结），见下几行。**2026-09-26（AI 输出片）起**：① 回复走 `aiChatStream` + `chat-live.ts` 的活气泡（每轮 `live?.remove()` 后新建，挂 `#lk-agent-msgs` 里，类名 `lk-agent__bubble lk-md`），收敛后再 `renderMsgs()` 整块重画（历史/DOM 逻辑不动）；② **删除 `numPredict: 900`**（`const cfg = { temperature: 0.7 }`），上限交给端点、`r.truncated` 时底部如实说「回复被模型的输出上限截断了」；③ `msgHtml()` 里 AI 气泡走 `mdToHtml()`（用户气泡仍是 `escapeHtml`）；④ agent 模式若从第一个 `{` 起就是动作 JSON ⇒ `placeholder('正在整理成动作…')` 遮住，**不能**用 `looksLikeToolJson()` 判（它要求整段合法，流式半截永远 false）；⑤ `window.__lkStreamLog` 是测试埋点（预建数组即生效，正式运行零成本）。**2026-09-26（第五十四轮）**：`SYS_HEAD` 第 5 条的口径改成「没有焦点时**一句话问他是哪一条**（能从【他最近做过的事】里挑出候选就点名问「是刚改的「X」吗」）—— **不许解释你看不到什么 / 你没有选中任何条目**」 | **2026-09-26（第 4 片 2C）起**：工具循环与提议卡片**抽成"宿主"共用一份** —— `export interface AgentRunHost { box; wrapClass?; innerClass?; bodyClass?; bodyStyle?; note(t, err?); rerender(); push(m); store }` 把「画在哪 / 存哪里 / 提示写哪」交出去，`export async function runTurn(text, host)` 是**一次提问的唯一实现**（系统提示 = `agentSystemPrompt(host.store, mode)`、流式活气泡取自宿主的类名、动作解析 / 纠错轮 / 轮数分档都只有一份），`panelHost()` 给浮层用、AI 页主会话自己拼一个。配套导出：`cardsHtml()`（提议卡片 HTML，两个入口共用，`data-prop-ok` 的下标就是 `cards` 的下标）、`agentCallRow(m)`（动作消息画成「用到动作：X」一行 —— AI 页不画它就会把动作 JSON 当聊天文字糊出来）、`export function agentCardClick(e)`（卡片「应用/忽略」的唯一处理函数）、`getAgentMode()` / `setAgentMode(next)`（**模式两个入口共享一份**：写状态 + 重画本层 + 广播 `lingkuang-agent-mode`）、`isAgentRunning()`。⚠️ 两条实测教训（`BUGS.md` 第五十七轮）：① 跑一轮期间 `busy === true` ⇒ 另一个入口的 `lingkuang-sessions` 监听会主动跳过重画，**跑完必须补一次广播**，否则同一格会话在另一边停在旧内容上（AI 页写完 8 条、浮层只画得出 6 条）；② 历史数组的指针会被 `adoptSessions()` 换掉 ⇒ **读它一律走 `hist()`**，别直接读模块变量 `history`。 **2026-09-26（思考片）起**：`runTurn()` 每轮先挂思考块再挂气泡、`onReasoning → think?.push`、落定 `think?.finish(r.reasoning)`；两条 assistant 入历史都带 `reasoning`；`msgHtml()` 给历史里的思考画 `thinkBlockHtml()`（排在气泡上面）。**2026-09-26（同日晚）起**：模型**压根不吐思考**时（用户实测在用的是 qwen2.5:7b，端点 message 里只有 role/content）界面上原本一点痕迹都没有 ⇒ `runTurn()` 用局部 `sawThink`（`onReasoning` 与 `r.reasoning` 两处置真）+ 模块级 `noThinkHinted`（**每个运行周期只说一次**），正常说完那一支若 `!sawThink && !noThinkHinted` 就在底部说明原因并点名建议换推理模型；**截断提示优先级更高**（先报截断），两个提示不互相盖。
| `src/ui/ai-sessions.ts` | AI 工具的**会话管理**（第 3.4 片）：`AiSession { id; name; role: 'main' \| 'character' \| 'perspective' \| 'director'; personaId?; links?; history; at; autoNamed? }`、`ROLE_LABEL`、`HIST_MAX = 120` / `LINK_TAIL = 8`、`adoptSessions` / `ensureSessionsLoaded` / `listSessions` / **`mainSession()`** / `activeSession` / `setActiveSession` / `createSession` / `renameSession`（主会话不可删）/ `setSessionPersona` / `removeSession` / `toggleLink` / `linkedSessions` / `pushMsg` / `clearHistory`（**原地清空** —— 换数组会让助手的指针指向旧数组）/ `sessionPrompt`（主会话一句人设都不加）/ `AI_SESSION_FRAME` / `setSessionSink`。落盘 `%APPDATA%\lingkuang\agent\sessions.json`（裸数组，能手看手改）。⭐ **2026-09-26 起 `role:'main'` 那条就是 Ctrl+K 的灵框助手**（用户：「其实我最开始的想法是主会话和助手指向的是同一个会话」）⇒ ① 同一份历史：助手不再写 `chat.json`（老文件只在主会话还空着时一次性迁入，之后留作老数据）；② `pushMsg` / `clearHistory` 末尾广播 **`lingkuang-sessions`**，助手面板与 AI 页各挂一个监听，把「同一格会话」**落到屏幕上**（各自生成中不重画，免得抹掉流式气泡）；③ `persist()` 在 `sink` 没注入时**自己兜底写** —— 助手浮层可以在 AI 工具没挂载时用 | **2026-09-26（思考片）起**：`normMsg()` / `pushMsg()` 保留 `reasoning` 并按 `REASON_MAX = 4000` 截一刀（推理模型一次思考几千字，不截会把 sessions.json 撑爆；截的是**落盘那份**，界面流式那份完整）。
| `src/ui/agent-context.ts` | 助手的**上下文打包**（原则仍是 ROADMAP §5 的「应用层查好再塞」，比让模型自己查可靠）：`setAgentFocus(f: AgentFocus \| null)` / `getAgentFocus()` / `buildContext(store, budget = 4000)`。`AgentFocus = { kind: 'entity' \| 'node'; world: string; id: string; title: string }`。内容 = 当前世界（与世界数）、当前时间线前 12 条节点（年/标题/种类）、**时间指针年份**（⚠️ 取 `fromEpoch(calendarOf(tl), cursor).anchor.year` —— 写成 `tp.year` 会静默 undefined）、各类型计数与名字、以及「正在编」那一条的字段与正文。喂数据点 = `src/ui/codex.ts` 的 `reportAgentFocus()`（`switchTarget()` 内 / `syncNewBox()` 尾巴 / dispose）；焦点**变了**才广播 `lingkuang-agent-focus`（换条目是 UI 状态、不一定动数据，光靠 store 订阅刷不动面板）。**2026-09-18（BUGS.md 第二十八轮）起**：另有两个导出 `setAgentFocusLive(v: boolean)` / `isAgentFocusLive()` —— 切走工具**只降级不清空**（`src/ui/codex.ts` 的 dispose 调 `setAgentFocusLive(false)`，块尾追加「他刚才在看这一条，现在切到别的功能去了」；`setAgentFocus(f)` 里 `nextLive = f ? true : live` ⇒ 重新上报即升回）；焦点块改称 `【创作者此刻打开的那一条】`、**排在【工作区】之后第二位**（原来在最末尾，小模型读完前面就不看了）、并带一行 vault 文件路径（`_设定/<类型>/<名字>.md`，规则同 `main.js` 的 `entityPath`/`nodePath`）；焦点为空时吐 `NO_FOCUS` 常量而不是留白。**2026-09-18（BUGS.md 第二十九轮）起**：`AgentFocus` 多一个 `view?: 'codex' \| 'timeline'`（`sameFocus()` 也比它），焦点块按 view 多一行「在哪：世界沙盘的时间线上（他刚点开这条看）」/「在哪：设定库工作台」；`buildContext()` 的 parts 顺序 = 工作区 → 焦点（`focusBlock` 或 `NO_FOCUS`）→ **`【他最近做过的事】`（`src/ui/agent-activity.ts` 的 `activityBlock()`）** → 时间线 → 设定。上报点 = `src/ui/codex.ts` 的 `reportAgentFocus()` 与 `src/ui/shell.ts` 的 `mountTimeline(... onSelect)`（沙盘点开节点时报 `view: 'timeline'`）；`src/tools/registry.ts` 的 `openTool()` 普通分支开头 `setAgentFocusLive(false)`（换工具只降级）。**2026-09-26（BUGS.md 第五十四轮）起**：新增 **`seedAgentFocus(f)`** —— 启动时由会话存档种一个**非 live** 的焦点（走 `setAgentFocus()` 会把 live 拉回 true ⇒ 谎报「正在编」，而工作台根本没在屏幕上）；`focusBlock()` 签名变 `focusBlock(store, ws)`、查表走 `owner = store.data.worldsets[f.world] ?? ws`，`f.world !== ws.name`（他切了世界、没换条目）**不再整块丢掉**、改多一行「⚠️ 这一条在**另一个世界**「Y」里」；无焦点时 `NO_FOCUS` 的口径改成「**不要向他解释你看不到什么** —— 按【他最近做过的事】/【当前时间线】挑最可能的那一条、一句话跟他确认，挑不出来就一句话问是哪一条」（旧口径「（没有：他没打开任何条目…直接问他指的是哪一条）」＋ `src/ui/agent.ts` 的 `SYS_HEAD` 第 5 条「就直接问他现在开的是哪一条」，模型会逐字执行成「我看不到你打开了哪个条目」—— 用户 2026-09-26 报的正是这句） |
...
| `src/ui/agent-tools.ts` | 助手的**动作表 + 文本 JSON 协议**（第 3 片）：`ToolCall { tool; args }` / `Proposal { tool; title; detail; apply(): { ok; note } }` / `findTool(name)` / `isWriteTool(name)` / `toolsPrompt()`（协议文本，进系统提示） / `parseToolCall(raw)`（**宽进严出**：整条回复以 `{` 开头才认；先 `normalizeCall(obj)` 认三种形状 —— 正统 `{tool,args}` / `{name|action|act, arguments|args|…}` / **单键「工具名当键」`{set_field:{…}}`**（用户实测踩到的形状）+ 参数摊平 + 值写成裸 JSON 字符串也认；**前面有正文、或工具名不认识 ⇒ null = 当聊天**，免得它举例说明时被误当调用） / `normalizeCall(obj)` / `looksLikeToolJson(raw)`（整条回复就是一个能 parse 的 JSON 对象但没能映射到动作 ⇒ `src/ui/agent.ts` 拿去**回头纠正一次**，不把裸 JSON 画给创作者看） / `runReadTool(call, store)` / `planWrite(call, store)`。只读 4：`list_entities{type?}`、`read_entity{name}`、`list_nodes{}`、`search{q}`；写入 5：`create_entity{name,type?,fields?}`、`set_field{entity,field,value}`、`append_doc{target:'entity'\|'node',name,text}`、`create_node{title,year,kind?,desc?}`、`rename_entity{entity,name}`。⭐ **不用 function calling**（本地 7B 的工具调用基本不可用）⇒ 让模型整条回复只写一行 JSON。⭐ **写入实现只有一条**：`apply()` 一律走 `src/store/actions.ts` 的 `addEntity`/`addNode` 或 `store.update`（id 与模板字段由它们负责），权限档只在 `src/ui/agent.ts` 的 `handleWrite()` 里决定「拒绝 / 出卡片 / 直接执行」。**2026-09-26（阶段 2A）起**：`toolsPrompt(mode)` 两个模式都注入（末句按模式分「一次提问只提一个动作」/「可以连着提多个动作」）；写工具多两个 —— `set_setting{key,value}`（白名单只认 `agentScope`/`agentAsk`/`agentMode`）与 `set_mode{mode}`（切模式走 `lingkuang-agent-mode` 事件，动作层不直接碰 `agent.ts` 的模块变量）；`Proposal` 多一个 `needsConfirm?: boolean` —— 自我提权那类动作（切 agent / 改宽范围 / 关掉询问）**永远**只出卡片，直接执行档也不放行；**2026-09-26（阶段 2B）起**：`planWrite(call, store, mode: AgentMode = 'agent')` 多收一个模式 —— 批量形态 `set_field{entities:[…]}`（同字段同值改一批，找不到的名字跳过并写在卡片上）与 `create_node{nodes:[{title,year,…}…]}`（一次建一串）**只在 agent 模式放行**，聊天模式收到就回一句「要批量改请先切到 Agent 模式」；两种形态仍然只出**一张**卡片 |
| `src/ui/agent-mode.ts` | 助手的**两种模式**（第 4 片，用户 2026-09-26：「我想让灵框平常是正常的聊天及工作，但是少数情况下有 agent 工作能力」）：`export type AgentMode = 'chat' \| 'agent'`、`MODES`、`MODE_LABEL`（聊天 / Agent）、`MODE_HINT`、`modePrompt(mode)`（**两个模式都返回内容** —— 2026-09-26 第二次改口后：chat 说「你有灵框内部的动作、但一次提问只做一个动作、写入会先变成提议卡片」，agent 说「可以连着做多步（最多 8 步）、批量活一次规划好」）。⭐ **模式不落盘、不进设置**：每次都从 chat 开始 —— 「少数情况」不该变成"忘了切回来"。落地方式 = 两个模式**都注入协议与权限段、都解析动作**，只有**回合数**按模式分（`src/ui/agent.ts` 的 `CHAT_ROUNDS = 1` / `AGENT_ROUNDS = 8`）；面板上换 `data-mode` / `.is-agent` / 分段按钮 `.is-on`。AI 也能用 `set_mode` 动作切模式，但那一步**永远**要创作者点「应用」（`needsConfirm`） |
| `src/ui/agent-perm.ts` | 助手的**权限闸门 = 两个正交旋钮**（第 4 片把第 2 片那个单档 `readonly\|confirm\|yolo` 拆开：范围 × 询问，借 DSH `permission` 预设的 sandbox/approval 两维）：`AgentScope = 'readonly' \| 'workspace'` / `AgentAsk = 'always' \| 'never'`（**类型定义在 `src/ui/settings.ts`**，避免循环 import；`Settings.agentScope` 默认 `'workspace'`、`Settings.agentAsk` 默认 `'always'`，合起来 = 原来的「逐项确认」）、`SCOPES` / `ASKS` / `SCOPE_LABEL`（只读/可写）/ `ASK_LABEL`（每次确认/直接执行）、`permName(scope, ask)` / `permHint(scope, ask)`（组合成人话，面板提示文案用）、`getAgentScope()` / `getAgentAsk()` / `getAgentKnobs()` / `setAgentScope(s)` / `setAgentAsk(a)`（变了才存 + 广播 `lingkuang-agent-perm`）、**`gateWrite(): 'deny' \| 'propose' \| 'allow'`**（每个写工具执行前必问；⭐ **先看范围**：`readonly` 无论问不问都是 `deny`，`workspace` 才按 ask 分 `propose`/`allow`）、`permissionPrompt()`（把权限写进系统提示 —— 不写，模型会一口答应「我帮你改好了」）。老键 `Settings.agentPerm` 在 `src/ui/settings.ts` 的 `loadSettings()` 里按 `OLD_PERM` 表一次性迁移（readonly→readonly+always / confirm→workspace+always / yolo→workspace+never）并删除旧键 |
...
| `src/ui/settings.ts` | 设置项的**存储与表单**（供应商与模型 / 画布偏好 / 设定演变 / **换条目转场**四组，存 localStorage `lingkuang-settings`；`loadSettings()` / `saveSettings()` / `renderSettingsInto(el, store)` / `activeProviderProfile()`）。**2026-09-26（供应商片）起**：面板是**左侧导航 + 四个分区**（模型 / 画布 / 转场 / 演变，`PAGES` + `showPage()`；**四页 DOM 一开始就都在、只切 `display`** —— 与工作台左树「两组都在骨架里只切显隐」同一条纪律：切回来草稿与滚动位置还在，也不会因为换页让下面整体跳一下），底部常驻「保存设置」；模型页 = 供应商清单（`#set-prov-list`，行 = 圆点「配好了没」+ 名字 + `模型 · 端点 · 协议` + 使用/编辑）+「＋ 添加供应商」菜单（`PRESETS` 里**还没加过的** + 永远在的自定义）+ 编辑卡（`#set-prov-editor`：名字 / 协议 / 端点 / Key / 模型选择器 / 设为当前在用 / 删除），`loadSettings()` 里 `migrateProviders()` 把老格式 `aiMode + baseUrl/apiKey/model` 折成一条档案并删旧键（`saveSettings` 之后落盘就是新格式）。转场那四个键：`motionSwap`(开) / `motionSpeed`(1×，除以时长) / `motionStagger`(10ms) / `motionEnterDx`(32px)，`codex.ts` 的 `playSwap()` 每次读取。**另有两把不上面板（第 4 片）的助手权限旋钮**：`agentScope`('workspace') / `agentAsk`('always') —— 面板在助手自己头上（`src/ui/agent.ts`），`loadSettings()` 里顺带把老单档 `agentPerm` 迁移成这两把并删旧键。**2026-09-26 起**：AI 卡片里的「模型」不再是手打文本框，而是**选择器**（`#set-model-btn` 触发器 + `#set-model-refresh`，展开的 `#set-model-menu` 里有 `#set-model-search` 搜索 / `#set-model-list` 清单行 / `#set-model-manual` + `#set-model-manual-ok` 手填兜底 / `#set-model-hint` 写状态与失败原因）—— 清单数据层在 `src/ui/ai-models.ts`。**当前值永远在单子上**（那行小字写 `当前在用`）；点一行只改内存里的 `s.model`（落盘仍走「保存设置」，与 Base URL/Key 同组）；改 Base URL / API Key / 模式会把提示换成「端点改了 —— 点「刷新」重新问一次」（清单按端点缓存，**不自动重拉**）；空清单时首次展开才去问端点 |
| `src/ui/session.ts` | **会话状态**（用户 2026-09-19：「我希望进入灵框时默认是上次关闭时的状态」）：`loadSession()` / `patchSession(p)` / `rememberTool(id)` / `rememberTarget(t)` / `lastTarget()` / `restoreSession(store)` / `watchSession(store)`，存 localStorage **`lingkuang-session`**（与 `lingkuang-settings` 同源；**不写 `worldbuilding.json`** ⇒ 不占撤销格、不进备份、不影响冷启动那几套逻辑；删掉这个键 = 回默认，无副作用）。只记三样：上次的**世界 + 时间线**（⚠️ 必须自己记 —— `src/main.ts` 的 `vaultToWorldData()` 只返回 `{ worldsets }`、`createStore()` 又固定取第一条，这两样启动时本来必丢）、上次打开的**工具**（`src/ui/shell.ts` 的工具栏点击里 `rememberTool(id)`；面板型工具不记 —— 它不接管主区）、设定库**正在编的那一条**（`src/ui/codex.ts` 的 `switchTarget()` 里 `rememberTarget`；`renderCodex()` 挂载时用 `lastTarget()` 落位，找不到那条就当没有）。⚠️ 恢复点在 `src/main.ts` 的 `renderShell()` **之后**、`watchSession()` **之前**（先后顺序是硬要求：先恢复后开记，否则启动默认值会先把存档盖掉），工具靠**点工具栏按钮**恢复（沙盘那条路根本不走 `openTool`）。沙盘缩放/平移/聚焦/非线性/剧情线、面板开合、窗口大小位置**故意不记**。`v !== 1` 或 JSON 解析失败一律当没有（手改坏了不该让应用打不开）。**2026-09-26（BUGS.md 第五十四轮）起**：`restoreSession()` 里把 `cache.target` 折成 `AgentFocus` 交给助手（`src/ui/agent-context.ts` 的 `seedAgentFocus()`，**非 live**）—— 位置在 `if (!s.tool || s.tool === 'sandbox') return;` **之前**：落点若不是设定库（AI 工作台 / 助手 / 沙盘），工作台不会挂载、**永远不会上报焦点**，助手只能答「你没打开任何条目」（守卫 `tools/e2e/agent-focus.cjs`，A/B 旧构建 6/11 → 11/11） |
| `src/ui/settings-panel.ts` | **悬浮设置面板**（用户 2026-09-13：「我希望设置面板是悬浮面板，而不是单开一个标签页」）：`openSettingsPanel(store)` / `closeSettingsPanel()` / `isSettingsPanelOpen()`。挂在 `document.body`、`position:fixed`、**主区一动不动**（正在编的条目与滚动位置都留着）；关法三种（× / Esc / 点遮罩空白），开关都广播 `lingkuang-panel` 让左栏按钮同步高亮。它对应 `src/tools/registry.ts` 里的 **`Tool.panel = true`**（面板型工具）—— 见 §工具宿主。**2026-09-26 起**卡片 = `760 × min(76vh, 620px)`（里面有左导航 + 分区，右侧分区自己滚，所以需要一个确定的高度） |
