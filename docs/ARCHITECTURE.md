...
- ⭐ **帧条的出入场走左右、跟正文一套**（用户 2026-09-14 第三轮：「**帧面板节点的出入场换成左右移动
  ...
  —— 出场 `rowsLeaveAndRemove`（`natural / 原地 → 0 / translateX(-32px)`，慢→快）、
  入场 **`rowsEnter(..., { start: 0 })`**（`0 / translateX(+32px) → natural / 原地`，快→慢）。
  ⚠️ 两个坑：① `rowsLeave()` 里 **`dy` 优先于 `dx`** ⇒ 想走左右就**不能给 `dy`**；
...
- ⭐ **已有的格子也要让位、整条时间线的高度也要演**（用户 2026-09-14 深夜：「已有的帧节点的位置变化也要平滑，
  ...
  新格子入场（`rowsEnter`）、框高（`smoothBoxHeight`）三件事 —— 明细见 `evolution-rail.ts` 文件表行。
