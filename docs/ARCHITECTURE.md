...
- ⭐ **左树的行级动效（2026-09-13，用户：「新建实体和节点时不是硬切换，而是从左侧滑入…其下的所有
  ...
  - ⭐ **收起时的退场也在这套原语上**（用户：「设定文件夹收起时无动画」→「文件出场动画改成入场的反向
    ...
    `leave = (ghosts) => rowsLeaveAndRemove(ghosts, { dy: 8, dur: 260, step: 22 })` ——
    **与 `rowsDropIn` 同序**（自上而下 0/22/44…，**不要 `reverse()`**：入场是从上往下一行行出来，
    退场倒着播等于波浪反向，与入场对不上）、方向相反（往上 8px）、曲线倒过来（慢→快）。
    `rowsLeave` / `rowsDropIn` 都有**错峰上限 `MAX_STAGGER = 240`ms**（`Math.min(i * step, maxDelay)`）：
    不限量时 44 行会让第一行等 **946ms** 才动，看着就是"卡住不动然后整片消失"。
- ⭐ **"里面的文件先消失，下面的文件夹再移上来"**（用户 2026-09-14：「应该是文件先消失，下面的文件夹再
  移上来，现在反了」）：`collapse()` 里 `flipDelayMs = rowsLeaveTotal(g.length, EXIT)`（= 单行 260ms +
  最后一行的错峰延迟，封顶 240ms），`renderList()` 把它传给 **`flipRows(..., { delay })`**。
