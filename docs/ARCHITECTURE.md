...
| `src/ui/ai-workbench.ts` / `roleplay.ts` / `tavern.ts` | AI 工作台 / 角色扮演 / 酒馆剧情推演。**2026-09-26（AI 输出片）起**：三处的对话气泡都改走 `chat-live.ts`（流式）+ `md.ts`（markdown），气泡行内样式抽成各自的 `bubbleCss(who)`、容器挂 `lk-md` 类；对话类请求**不再带 `numPredict`**（原来角色扮演 400 / 酒馆 500 / 工作台 500 —— 那就是长回答被切半句的原因） |
