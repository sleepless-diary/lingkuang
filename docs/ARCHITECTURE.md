...
- **挂点**：切工具/开面板 = `src/tools/registry.ts` 的 `openTool()`（唯一入口，覆盖工具栏点击与快捷键；
  ...
  **只入场不退场**：退场要等 animationend 才能 resolve，破坏性操作的 Promise 不该为观感延迟）；
  ...
  （旧 `#cx-new-node-btn` 删掉），它按 `mode` 分派 `createEntities()` / `createNodes()`，文案在
  「＋新建实体 ↔ ＋新建节点」之间用 `rollText()` 滚（`.lk-roll` / `.lk-roll__t`，见 `motion.ts` 行）。
  守卫：`codex-smooth-switch.cjs` ★13g（四个小标签）/ ★13d（滚字关键帧）/ ★13d2（旧字克隆与动画都收干净）。
