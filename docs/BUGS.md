...
### 修法：三层，从便宜到贵（`dropStaleNodeFiles` / `dropStaleEntityFiles`）

`kill` 一律**按 id 命中才删** —— id 相同即同一个节点，那份是被应用忽略的旧副本，删它不丢用户可见内容。

- ⓪ **同目录按 id**：改名后的残留（旧文件还在同一个文件夹、只是换了名字）。**修复前就有的行为，保留。**
- ① **别的种类文件夹里同名**的那份：换种类的残留。应用自己造的残留必定同名（换种类只换文件夹），
  所以这一层同时**自愈升级前就留在盘上的旧残留** —— ★8 就是测这个（手工把旧残留放回旧文件夹，
  应用自己清掉，收敛后界面说的种类与活下来的文件一致）。
- ② `deep`（目标路径还不存在 = 搬到没住过的地方）时按 id 扫这条时间线全树：
  覆盖「改名 + 换种类同时发生」（两个字段落进同一次 400ms 防抖落盘就会这样）。
  只在这一次跑，是为了别让每次落盘都全树解析（`writeAll` 遍历全部节点 ⇒ O(节点数²) 次读盘）。
- 实体侧对称收敛：`_设定/**` 全树扫描从第十六轮的「每次写都做」改成同一条三层规则。
...
换到干净起点后 `codex-node-tab` 回到 **15/15 ×2**。
（这也是「⓪ 不能丢」的证据：它正是旧代码里清同目录残留的那一步，我在第一版里为省时把它关掉了。）

### 验证

- 新测试 `kind-change-stale-file.cjs` **10/10 ×2**（干净起点）
- `codex-node-tab` **15/15 ×2**（干净起点）、`editor-props-panel` **9/9**、`data-load-clean` **6/6**
- `entity-vault` **17/17**、冷启动 **PASS**、`startup-materialize-entity` **6/6**
- `codex-switch-target` **7/7 ×3**
- `node --check main.js` / `tsc --noEmit` / `vite build` 全 exit 0

### 仍在案的（未修）

- **⓪ 的代价是既有的 O(N²)**：每次写盘都把**目标文件夹**里每个 `.md` 读出来比对 id，
  而 `writeAll` 每次落盘遍历全部节点 ⇒ 几百个节点的世界、每次自动落盘几万次读盘。
  这一轮**没让它变差**（修复前就是这样），但也没修。方向（下一轮可做）：
  ① 让 `scanTimelineDir` 顺手记 `id → 源文件路径`，写盘时按 id **O(1)** 删旧文件，⓪ 连带全树都可以不要；
  ② 或把「同 id 只留一份」的整理挪到**扫描期**一次做完（扫描本来就读了全部文件，零额外 I/O）——
  但那等于「扫描会动用户文件」，要单独设计（移进回收站、可恢复）。
- **同一条时间线里同 id 两份并存时，赢家由 readdir 顺序决定**（NTFS 按 UTF-16 序）。
  这既是本 bug 的传播机制，也是上面「脏目录」那种歧义的根源；①/② 任一做掉都能顺带消掉它。
