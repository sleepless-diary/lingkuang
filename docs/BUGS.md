...
先排掉「没重启」：他的正式应用启动于 21:13:19，而 `app-dist/` 构建于 21:12:51 ⇒ 跑的就是含思考显示的那一版。
于是只剩一条可能：**模型不吐思考**。实测（读他正式应用 localStorage 里「当前在用」的供应商，再各发一次极短流式请求）：

| 供应商 / 模型 | 端点的思考字段 | 结论 |
| --- | --- | --- |
| 本地 Ollama · qwen2.5:7b（**当时在用**） | `message` 只有 `role` / `content` | 模型本身没有思考 |
| 本机 qwen3:14b / qwen3:32b（对照） | `message.thinking` ✓ | 切过去就能看到 |
| 设置里的 DeepSeek · deepseek-flash | HTTP 401「Your api key: ****2551 is invalid」 | 他说的「稍后删了重建」的那把已失效 |

⇒ 第五十八轮的功能是好的，只是**不会思考的模型，界面上一个字都不可能出来**。

### 真缺口（本轮修的）
「看不到」这件事本身没有反馈：不吐思考时 `liveThink.finish('')` 直接把空块摘掉，屏幕上连「思考」这一栏都没有，
用户分不清是**功能没做**还是**这个模型没有**。修法（`src/ui/agent.ts` 的 `runTurn()`）：
局部 `sawThink` 记这一问有没有收到过一个字的思考（`onReasoning` 与 `r.reasoning` 两处置真），
正常说完那一支且 `!sawThink && !noThinkHinted` 时底部说明一次：
「模型：X · 它不吐思考过程（这个模型不是推理模型）—— 想看得见思考，去「设置 → 模型」换成会推理的，比如本机的 qwen3:14b」；
模块级 `noThinkHinted` 保证**每个运行周期只说一次**。被截断那一支优先级更高（先报截断），两个提示不互相盖。

### 教训
- 「功能没效果」的报告，先分清**屏幕状态**与**数据来源**：这次断点在模型侧（端点根本没这个字段），不在渲染侧。
  诊断要读**用户正式应用当前在用**的供应商（`%APPDATA%\lingkuang\Local Storage\leveldb` 里那串 UTF-16 的
  `lingkuang-settings`；抓取时要从 `"providers"` 往前找最近的 `{` 逐个配平解析，否则会抓到旁边别的 JSON），
  别只看代码和测试实例；脚本里**永不打印 apiKey**。
- 用户能看到「思考」的前提是模型会思考：本机已装的 `qwen3:14b` / `qwen3:32b` 都吐 `thinking`；
  `qwen2.5:7b` 这类普通对话模型永远不会。

### 守卫
`tools/e2e/agent-stream.cjs` 10 → 12 项：★1b（不吐思考那一轮底部出现说明、且不是错误样式）、
★3b（这条说明只出现一次，后面不吐思考的回合不再重复念）。
**A/B：旧构建 11/12（★1b 读数 `{text:"", err:false}`）→ 新构建 12/12**；回归 agent-panel 21/21、
agent-tools 19/19、agent-mode 12/12、agent-main-session 13/13。

### 顺带
`tools/e2e/probe-focus2.cjs` 里等「正在思考…」消失的判据原本写成 `indexOf('思考') < 0` ——
新提示正文里就有「思考」两字，会让这个探针永远等不到收敛；改成精确的 `'正在思考'`。
...
### 用户报的（原话）
「ai：另外你说的「这个／这条」我这边看不到你打开了哪个条目——你此刻没有选中任何条目（界面停在 AI ...）要怎么避免 ai 出现这种回答」

### 三层病根（逐层钉住，不是猜）
1. **焦点只有「工作台在屏幕上」时才上报**：`src/ui/codex.ts:730` 的 `reportAgentFocus()`（由 `switchTarget()` 与骨架 render 尾巴调）与 `src/ui/shell.ts:53`（沙盘点节点）是**唯一**两个上报点。启动落点若是 AI 工作台 / 助手 / 沙盘，设定库压根不挂载 ⇒ 没有任何东西上报 ⇒ `focus === null` ⇒ 上下文里只剩 `NO_FOCUS`。
   而 `src/ui/session.ts` 明明记着「上次在编的那一条」（`lastTarget()`，由 `src/ui/codex.ts:750` 的 `rememberTarget()` 写），**却没交给助手** —— 这就是用户那一幕的直接来由。
2. **焦点在另一个世界时被整块丢掉**：`src/ui/agent-context.ts` 的 `focusBlock()` 开头 `if (!f || f.world !== ws.name) return '';` —— 他切了世界、没换条目，助手看到的就是「没打开任何条目」，而事实是他打开着一条。
3. **提示词教它念局限**：`NO_FOCUS` = 「（没有：他没打开任何条目。他说「这个 / 这条 / 当前 / 我打开的文件」时，直接问他指的是哪一条，不要拿世界名或时间线名糊弄…）」＋ `src/ui/agent.ts` 的 `SYS_HEAD` 第 5 条「若那一栏写着「没有」，就直接问他现在开的是哪一条」⇒ 模型**逐字照做**，答出用户引的那句。

### 修法
- `src/ui/agent-context.ts` 新增 **`seedAgentFocus(f: AgentFocus | null)`**：设焦点但**强制 `live = false`**（走 `setAgentFocus()` 会把 live 拉成 true ⇒ 谎报「正在编」，而工作台根本没在屏幕上）。
- `src/ui/session.ts` 的 `restoreSession(store)` 里，把 `cache.target`（实体 / 节点，节点自带 `tlId`）折成 `AgentFocus` 并 `seedAgentFocus(...)`；位置在 `if (!s.tool || s.tool === 'sandbox') return;` **之前**（和工具落点无关 —— 沙盘/面板型工具那条早退也要种）。查不到那条（被删 / 换了世界 / 换了数据目录）就什么都不做。
- `focusBlock(store, ws)`：签名带上 store，查表走 `owner = store.data.worldsets[f.world] ?? ws`；`f.world !== ws.name` 时**不再丢块**，改为多一行「⚠️ 这一条在**另一个世界**「Y」里（他此刻的世界是「X」——别把它当成当前世界的条目）」，文件路径也用它自己的世界名。
- `NO_FOCUS` 改成：「（没有：这会儿没有正在编的条目。**不要向他解释你看不到什么** —— 他说「这个 / 这条 / 当前 / 我打开的文件」时，先按【他最近做过的事】与【当前时间线】挑出最可能的那一条、一句话跟他确认（比如「你说的是刚改的「X」吗？」）；挑不出来就直接一句话问他是哪一条；不要拿世界名或时间线名糊弄，也**不要写「我看不到 / 你没有选中」这类解释**）」
- `src/ui/agent.ts` 的 `SYS_HEAD` 第 5 条：把「就直接问他现在开的是哪一条」换成「就一句话问他是哪一条（能从【他最近做过的事】里挑出候选，就点名问「是刚改的「X」吗」）：别拿世界名、时间线名糊弄，**更不要解释你看不到什么 / 你没有选中任何条目**」；降级档那句也补「（同样别解释你「看不到」）」。

### 守卫 `tools/e2e/agent-focus.cjs`（11 项）与 A/B
- 手法：先在工作台点中种子实体（让 `session.target` 记下它）→ 把 session 的 `tool` 改写成 `'ai'` → `location.reload()`（**复现用户那一幕：重启后落点是 AI 工作台、设定库没挂载**）→ Ctrl+K 开面板，读 `#lk-agent-focus` 的 chip 与 `#lk-agent-ctx` 全文。
- ★0/★0b 前置 / ★1 面板开出来了且设定库确实没挂载 / ★2 chip = 「最近在看：银发少女」（**不是**「没打开条目」）/ ★3 上下文里有那一条（名字 + `_设定/` 路径）且**没有**「没打开任何条目」/ ★4 系统提示词有「更不要解释你看不到什么」、**没有**旧那句「就直接问他现在开的是哪一条」/ ★5 无焦点时 `NO_FOCUS` 新文案在、旧句不在 / ★5b 无焦点时 chip 如实写「没打开条目」（不谎报「正在编」）/ ★6 **焦点在另一个世界**（用 `#lk-world-new` 新建世界；`src/store/actions.ts` 的 `addWorld()` 会顺手 `setActiveWorld(新世界)`）时上下文仍有那一条 + 「另一个世界」/ ★7 探针世界已删、当前世界切回原样 / ★8 无异常。
- **A/B：旧构建 6/11 → 新构建 11/11**（连跑两次读数一致）。旧构建的关键读数：`chip:"没打开条目"`、`hasNoFocusLine:true`、`oldRule:true`、★6 `kept:false / worldNote:false`；新构建：`chip:"最近在看：银发少女"`、`hasNoFocusLine:false`、`newRule:true / oldRule:false`、★6 `kept:true / worldNote:true`。
- ⚠️ 套件为了验「跨世界」那条**会新建一个世界**（「焦点探针世界」），断言完**右键 → 删除**并把当前世界切回去：残留世界会让 `localStorage['lingkuang-session']` 指着它，别的套件启动时落在空世界 ⇒ 假 FAIL（同类污染见第三十二轮 / 第四十八轮那两条「空时间线被会话恢复选中」的教训）。

### 教训
- **断言里的中文文案要逐字对**：★5 第一版写「不要解释你看不到什么」，而实现里是「不要向**他**解释你看不到什么」⇒ 白挂一条。读数特征是「新句不在、旧句也不在」—— 看到这种组合先怀疑自己的断言串，而不是实现。
- 「助手看不见」这类问题**先分清是哪一层**：产品真的不知道（焦点空 / 跨世界被丢）⇒ 补数据；知道却不许说 ⇒ 改口径。两层都在时只改一层，就会变成「它明明知道却说你没打开」或者「口径很好但没料」。
...
### ① 病根：交叉淡化让第二套网格**长期**挂在屏上（不是"换档那一瞬才挤"）
- 探针 `tools/e2e/probe-scale-handover.cjs` 逐格实测（61 格）：一屏 **28~41 根**粗刻度，其中邻居档
  **18~32 根**；"两套不同单位"只有 3/61 格，但**同单位的嵌套网格**（年↔年 的 1/2/5/10 阶梯）
  几乎每一格都在（ghost 18~20 根、不透明度最高到 0.947）⇒ 用户看到的"密集"主要是这一类。
- 两套数字交错时最小间距能到 **~20px**，比一个「312年」还窄 ⇒ 糊成一片。
  A/B（旧构建，e2e 直接量）：同一行里「**164年**」与「**1月**」横向叠了 **19.7px**。
- 结构性原因：`GRID_NEIGHBOR_F = [1.15, 1.45, 1.9, 2.6]` 采样宽 + `GRID_MIN_W = 0.03` 门槛低 ⇒
  邻居权重掉到 0.03 才不画 ⇒ 屏上长期有 1~2 套"半隐"的邻居。

### ② 修法：`paintScale()` 尾段加一遍**屏幕盒子贪心去重**（`src/ui/timeline.ts`）
- 新的执行顺序：建/复用元素 → 写 `left` + ghost 类 → 顺序修正 → **尾段**：量文字宽度 → 按行去重 →
  写不透明度（旧版是在主循环里顺手写 opacity，没有"全局视野"就没法去重）。
- 判据按**行**分开：`.tl__axis-label` 在 `top:4px`（线右侧 `left:3px`）、`.tl__axis-prev` 在 `top:14px`
  （线正下方、`translateX(-50%)` 居中）—— 两行上下错开本来就不打架，并成一个盒子会**过量地砍**。
- **当前档先占位**（`it.line === true`），邻居档（ghost）撞上就**整根隐掉**（文字 `opacity: 0`；它的线
  本来就是 `border-left-color: transparent`）⇒ 剩下的第二套数字变成"隔一段留一个"，既看得出在换档、
  又不糊。间距阈值 `TICK_MIN_GAP = 3`px。
- 元素**一律留着**（不增删 DOM）⇒ 仍是**纯函数**（同一视图状态同一个结果）、不播动画、不闪；
  `scale-motion.cjs` ★4b（可逆性）复跑 `pairs 166 / worst 0 / differ 0` 证明没被破坏。
- 宽度：`textW: Map<string, number>` **按文字缓存实测宽度**（`getBoundingClientRect().width`，一段文字
  只量一次）；`scaleSpans: WeakMap<HTMLElement, {cur, prev}>` 在 `rememberLabels()` 里一起记账
  （＋省掉每帧的 DOM 查询 —— 仓库对每帧布局/查询量敏感，见 `causes-line.cjs` ★3）。

### ③ 守卫 `tools/e2e/scale-motion.cjs` ★8（10 项）与 A/B
- ★8 = 逐帧按**行**（`Math.round(rect.top)`）扫所有**看得见**（`opacity > 0.05`）的文字盒子，任意两个横向
  重叠 > 1px 就 FAIL。**A/B：旧构建 `maxOver 19.7`（`164年` vs `1月`）FAIL；修复后 `maxOver 0` PASS。**
- 同轮修掉两条**既有测试真值**问题（都不是产品回归，但会假 FAIL）：
  - **★4**（屏幕两端渐隐）原来无条件断言 `midMin === 1`。交叉淡化让同屏有第二套网格，而"同一个 key 被
    两套产出取并集"会把当前档的一部分数字抬到高于自己的权重 ⇒ 干净 fit 正好落在档位区间边缘时，
    mid 里同时有 0.775 与 0.631 ⇒ `midMin 1` 不成立。修法 = 量之前**先沿缩放轴往外滚**（最多 7 格、
    两个方向都试），落到 **`ghost == 0`（一个邻居档都没有）**的档位再量 —— `labelWindow` 的前提就是
    "换档权重 = 1"。修后 `midMin 1 / monotone true / edgeMin 0.135`。
  - **★7**（换档不是硬切）第一版按**单位文字**判"两套"（年/月/日/时/分）⇒ 61 格里只有 3 格能撞到，
    撞不撞得上还取决于干净 fit 落在档位区间的哪儿（换个夹具/多播一个 seed 就换轨迹）⇒ 假 FAIL
    （实测 `mixed 0`）。绝大多数换档其实是**同单位的嵌套网格**（年↔年 的 1/2/5/10），按文字根本看不见。
    改成按产品自己的记号判：**邻居档 = `.tl__axis-tick--ghost`**（当前档 = 没这个类的），逐帧读
    "当前档最亮 / 邻居档最亮"，判 `gMax ∈ [0.15, 0.9)`（半亮）且当前档也可见。修后
    `mixed 16 / twoFull 0 / minTop 0.731 / animFrames 0`。
- 📌 **跨套件污染（踩了两次）**：同一台测试实例连着跑两套会假 FAIL —— `localStorage['lingkuang-session']`
  记着上一套留下的工具/时间线，下一套的"启动态"就不是它以为的那个（本轮 `timeline-scale` 10/12、
  `storyline-focus` 1/13 都是这么来的，**清掉 `%TEMP%\lk-evault2\userdata` 后分别 12/12、13/13**）。
  每套件单独起实例**不够**，还要清 userdata。
- 回归（每套件单独起实例 + 清 userdata）：`scale-motion` 10/10、`timeline-scale` 12/12、
  `storyline-focus` 13/13、`causes-line` 7/7、`nonlinear-pan` 7/7、`scale-hidden-mount` 5/5、
  `motion-switch` 25/25；`node --check main.js` / `npx tsc --noEmit` / `npx vite build` 全绿。

## 第四十七轮（2026-09-26）· 换档交叉淡化：文字不透明度 = **缩放轴**的纯函数（第四十六轮 ② 的落地）

> 用户规格（原话）：「**把整个缩放尺度当成一个 x 轴，在轴上时，文字的不透明度图像类似于一个正态分布的
> 图像（100% 不透明度的占比要长一点），每个文字依照对应的 x 计算当前的不透明度**」；
> 动手前把唯一那个分叉问清了 —— **A 交叉淡化**（新旧两套同时在、各约 50%），不是"先出后进"；
> 屏幕两端渐隐**留着但减淡**（用户：「留着，但是左右的淡出和淡入强度降一点（100% 不透明度范围加长）」）。

### ① 落地（`src/ui/timeline.ts` + `src/style.css`）
- `quantStep()` → **`quantStepAt(sp)`**（按任意 spacing 求档位）；原来的无参包装没了调用方，删掉。
- `renderScale()` 里那 100 行刻度生成抽成 **`gridItems(unit, stepSec, majorOnly)`** —— 两套网格的生成规则
  完全一样（历法进位 / 全局原点相位 / 聚焦剔空隙），只是 `unit`/`stepSec` 不同；`majorOnly` = 邻居网格
  只要主刻度（它的**线**一律隐身，小刻度画了也看不见，白占 DOM 与每帧写样式的开销）。
- 新增 **`gridRange(unit, stepSec, spInside)`**：这套网格在 spacing 轴上"会被 `quantStepAt` 选中"的那一段 ——
  `same()` 从 `spInside` 向两侧 ×1.1 扩到第一个"不是它"的点，再二分 30 次；按 `unit|stepSec` 缓存一辈子。
- 新增 **`gridWeight(unit, stepSec, spInside, spNow)`**：`u` = 当前 spacing 到区间中点在 `log` 轴上的距离 ÷ 半宽
  ⇒ 区间内 `u ∈ [0,1]`、**交接点上 `u = 1`**；`u ≤ GRID_PLATEAU(0.6)` 恒 1，之后
  `exp(−ln2·((u−p)/(1−p))²)` ⇒ **u=1 处正好 0.5**（两套各半、加起来 ≈1 ⇒ 不会出现"两边都看不见"的空白）。
- `renderScale()` 每帧收**候选网格** = 当前档 + 按 `GRID_NEIGHBOR_F = [1.15, 1.45, 1.9, 2.6]` 两个方向采样到的
  邻居，权重 < `GRID_MIN_W(0.03)` 的不画；同一个 key（同 unit 内嵌套网格里"名字没变"的数字）取**并集**
  `1−(1−w₁)(1−w₂)`（1980 年在 1年/2年 两套里都成立 ⇒ 交接点上它该是 0.75，不是 0.5 —— 这个数字并没有"要走了"）；
  最后**按 `left` 排序**再交 `paintScale()`。
- `paintScale(items)` 的 item 多两个字段：`w`（换档权重）、`line`（是不是当前档）。文字
  `opacity = labelWindow(位置) × w`；邻居档的刻度加类 **`.tl__axis-tick--ghost`**
  （`src/style.css`：`border-left-color: transparent`）⇒ **屏上永远只有一把尺子的线**。
- 屏幕端那套（用户要求减淡）：`LABEL_PLATEAU` 0.34 → **0.44**（中间 88% 恒 1）、尾巴 `exp(−4t²)` → **`exp(−2t²)`**、
  贴边下限 **0.135**（不再是 0.018）。
- ⚠️ **两个当场踩到的坑**：
  ① `gridRange` 第一版拿"当前 spacing"当"已知落在网格内"的点 ⇒ 邻居网格 `same()` 为假 ⇒ 返回 null ⇒
     权重退化成 1（两套都满不透明度 = 第四十二轮那两把尺子又回来了；探针实测 `套数=2` 且两边 `max op = 1`）。
     修法 = 候选网格各自带 `inside`（当初采样到它的那个 spacing）。
  ② 不排序就交 DOM ⇒ 交接点上两套网格的数字是"每套内部有序、两套前后相接"⇒ **不是从左到右**：
     谁按 `querySelectorAll` 的顺序读都会看到负间距（A/B 实测 `timeline-scale` ★1/★1b/★2 一起 FAIL、
     `gaps` 里冒出 `-370`，`minGap` 甚至 -822）。

### ② 守卫（`tools/e2e/scale-motion.cjs` 9 项）与 A/B
- **★7（新功能）**：40 格缩放里 `mixed ≥ 1`（有帧上两套档位同在、且**两套都在淡的中途** max ∈ (0.15, 0.9)）、
  `twoFull = 0`（从不两套都 ≥0.9 = 不重演"两个重叠的标尺"）、`animFrames = 0`（零动画）、
  `minTop ≥ 0.45`（只看中间段 x ∈ [0.2w, 0.8w]：任何一帧都至少有一套 ≥0.45 = **不许出现空白**）。
  **A/B：改动前 `mixed 0`（硬切、任何一帧只有一套）⇒ 8/9 FAIL；改动后 9/9**（读数 `mixed 2`、
  `twoFull 0`、`worstSum 0.93`、`minTop 0.634`、`animFrames 0`）。
- **★4b（重写）**：**不透明度只由视图状态决定**（纯函数 ⇒ 可逆）—— 从干净 fit **放大 12 格**、每格记
  `文字|屏幕位置 → 不透明度`，再**缩回 12 格**同样记一份，按"同一格"逐一比对。
  实测 **`pairs 184 / worst 0 / differ 0`**（连跑两次一致）。
  为什么不用"逐帧 |dOp| ≤ K"：那个指标同时受**视图速度**影响（测试实例 rAF 不受 vsync 限制，机器一慢就假 FAIL）。
  ⚠️ 写这个断言踩了三个坑（都别再踩）：❶ 只按**文字**存 map —— 月/日档的 `.tl__axis-label` 只有「11月」
  （年份在 `.tl__axis-prev` 里），同屏好几个「11月」互相覆盖 ⇒ 比的是不同刻度；❷ 两趟数组**不同构**
  （`back` 多推了一次快照 ⇒ 整体错开一整格 = 20% 缩放 ⇒ 一片"差 0.487"的假 FAIL）；❸ 用"整屏位置签名完全一致"
  当对齐条件太脆（屏端多一根/少一根刻度签名就变）。
- **★4 改名/改注释**：现在它只量"屏幕两端渐隐"那一半（换档权重是另一半）。
- `timeline-scale.cjs` / `storyline-focus.cjs` / `scale-motion.cjs` 的取样统一加
  `:not(.tl__axis-tick--ghost)`：量的必须是**当前档**的几何 —— 两套混着读，"等距 / 等差 / 落在全局网格上"
  当场崩（这也是 A/B 里 ★1/★1b/★2 一起 FAIL 的原因）。
- **回归全绿**：`timeline-scale` 12/12、`storyline-focus` 13/13、`scale-motion` 9/9（连跑两次）、
  `scale-hidden-mount` 5/5、`causes-line` 7/7、`nonlinear-pan` 7/7、`motion-switch` 25/25；
  `node --check main.js` / `npx tsc --noEmit` / `npx vite build` 全绿。
- 📌 **夹具坑（又踩一次）**：`timeline-scale` 必须**单独**跑（`reset-entity-vault` + `seed-node`）；
  先跑 `seed-storyline-focus` 会在 `worldbuilding.json` 里留下空时间线 `tl-side`，被会话恢复选中 ⇒
  ★0b 直接 `has:false`、★4 `common:[]`（看着像回归，其实是被污染）。另外**不能整棵清空**
  `%TEMP%\lk-evault2`：`reset-entity-vault.cjs` 与各 seed 脚本都要求 `worldbuilding.json` / `formats.json`
  **已存在**（先起一次应用让它自建，或者只删 vault 与 userdata）。

## 第四十六轮（2026-09-26）· 缓动被 600ms 兜底半路掐掉（**用户自己诊断对了**）+ 需求纠正「出入场 = 整体文字按**缩放轴**」

> 用户原话：「**等等等等，原来你改的是边缘的出入场吗，我的出入场意思其实是整体文字的出入场，
> 我们现在缩放时文字不是会隐藏其他尺度的文字吗，但是文字隐藏和出现是硬切的，而且我发现缩放时的
> 卡顿是什么了，在缩放尺度边缘时标尺缩放的缓动没生效好像**」。
> （后半句是本轮修掉的 bug = ①；前半句是需求纠正 = ②，**尚未实现**。）

### ① 缓动半路被掐（已修）
- 病根：`src/ui/timeline.ts` 的 `kickEase()` 启动缓动时挂了一条 `setTimeout(..., 600)`，
  **到点就无条件** `cancelAnimationFrame(easeRaf) + snapView() + render()`。它本意只防「rAF 被降频/暂停时
  平滑循环停在半路」（后台窗口 / 未聚焦 / 节流），但**连滚多格时缓动本来就可能跑过 600ms** ⇒ 兜底把
  **还在跑的**缓动掐掉、一帧内直接落值。用户说的「缩放尺度边缘」正是这种场景：攒到缩放上限
  （`src/ui/timeline.ts` 的 `const next = Math.min(1e8, Math.max(0.05, targetView.spacing * factor));`）
  本来就要连滚几十格。
- **实测**（新增探针 `tools/e2e/probe-ease-edge.cjs` = 逐帧读时间指针的 `left`（`cursorEl.style.left = timeToX(t)`，
  `src/ui/timeline.ts` 的 `updateCursor` 里那句）+ 给 `setTimeout`/`clearTimeout`/`cancelAnimationFrame` 打点；
  ⚠️ 必须先 CDP `Emulation.setFocusEmulationEnabled {enabled:true}`，否则 `noSmooth()` 为真、滚轮当帧落值，
  根本没有缓动可看）：
  - 修复前：`61ms arm600` → **`664ms caf`**（一帧内 view 从 `-34,196,900` 跳到 `-125,926,000`，**9,172 万 px**）
    → `675ms arm600` → **`1278ms caf`**（`-19,629,000,000` → `-20,019,500,000`，**3.9 亿 px**），运动 1281ms 就断。
  - 修复后：**`caf` 0 次**，缓动一路跑到底（持续到 2156ms 才自然收敛）。
- 修法：把「按时间无条件落值」换成**看门狗** —— `src/ui/timeline.ts` 新增 `easeWatchdog` / `lastFrameAt` /
  `EASE_STALL_MS = 250`；`step()` 每帧刷新 `lastFrameAt`；看门狗每 250ms 醒一次，**只有 250ms 内一帧都没出**
  才 `cancelAnimationFrame + snapView() + render()`（后台/未聚焦/节流那件事照旧防得住 —— 旧注释里
  「平移 180px 只走了 27px 就停住」正是它要接的）；缓动正常跑完走 `clearTimeout(easeWatchdog)`。
  ⇒「缩放时标尺有轻微卡顿」与「在缩放尺度边缘缓动没生效」是**同一个病根**，一起没了。
- 回归：`tools/e2e/scale-motion.cjs` **8/8**、`tools/e2e/timeline-scale.cjs` **12/12**
  （两套都带 `LINGKUANG_TEST_WINDOW_NOFOCUS=1` ⇒ 走"当帧落值"那一支，**看不到这条改动** —— 探针才是证据）。

### ② 需求纠正：出入场 = **整体文字**按**缩放轴**算，不是屏幕边缘变暗（✅ **已实现 → 见第四十七轮**）
- 第四十五轮（上一轮）我把"正态分布"用在了**屏幕水平位置**（中间 68% 屏宽恒 1、两头尾巴），
  用户当场反问「**原来你改的是边缘的出入场吗**」⇒ 他要的是**缩放轴**上的整体文字出入场：
  「缩放时文字会隐藏其他尺度的文字，但文字隐藏和出现是硬切的」= **换档（年↔月↔日↔时↔分，
  以及同一档里 niceStep 的 1/2/5/10 阶梯）时一整批数字硬切**，要用**随缩放值算的不透明度**交叉淡化。
- ⚠️ **分叉（已问用户 = A）**：用户"正态分布 + 长平台"的规格读出来是**两套数字同时在、各约 50%**
  （交叉淡化）；但第四十二轮他明确报过「**会暂时出现两个重叠的标尺**」并要过「先出后进」——
  两者在视觉上互斥，**动手前问清了：选 A 交叉淡化**（屏幕两端那套则"留着但减淡"）。
- 实现骨架（两种选法都适用）：权重 = **缩放值的纯函数**（在 `log(spacing)` 上算；每套网格
  `unit + stepSec` 有自己的区间，区间中部恒 1 = 用户要的"100% 占比长一点"，两端降到 0.5 ——
  正好落在交接点上，两套加起来 ≈1 ⇒ 不会出现"两边都看不见"的空白；刻度 key 里**不含 `stepSec`**
  这一点正好让同一 unit 内"名字没变的数字"自动复用同一元素，避免重复画）；
  **线仍只留当前档那一套**（非当前档的刻度线隐身，只有数字淡）—— 不能重演第四十二轮「两个重叠的标尺」。
