...
- **做法**：`src/ui/inspire.ts` 的 `renderChar()` 末尾 `cascadeIn(result, 50, 720, 120)`
  （`result` = 卡片网格 `#insp-result`）—— 13 张正好排成 `120·170·…·720ms` 的阶梯。
  步长 50、封顶 720 是刻意的：封顶太早（如 500）时后面几张会**同时**冒出来，看着像整块。
  触发时机 = 打开工具（`inspire.ts:227`）/「重新生成」（`:201`）/「组合 N」加载（`:193`）三个**显式动作**；
  锁定（`:134`）与改词条数（`:148`）按既有设计**不重建卡片**，所以不会重播。
...
- `tools/e2e/motion-switch.cjs` 扩到 **22 项**（新增 ★18 卡片逐个错峰 + 卡片区自己不整块淡入、
  ★19「重新生成」重播、★20 锁定不重建卡片、★21 减少动效下卡片降级）—— **22/22**
