...
- `editor-tree.cjs` **28/28**（`seed-editor-tree.cjs` 播种：`事件` 有 2 个节点 / `战斗` **空**；
  实体类型 `角色` 有 1 个实体 / `地点` **空**）；A/B 7/28。`%TEMP%\lk-edtree` + 端口 9704。
- 视觉取证：1440×900 截图确认 `测试世界观 → 主线 → 事件 2 / 战斗 0（置灰，展开写"这个结构体还没有节点"）`
