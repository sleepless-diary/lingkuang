...
### 验证
- `tools/e2e/seed-evolution.cjs` + `entity-evolution.cjs` **27/27**：
...
- `tools/e2e/cold-start-evolution.cjs` **8/8**：重启后帧还在、物化正确（初稿 / 末版两套值）、
  正文按版本取、默认落点仍是最近的那一帧。
...
- 手动模式在"没有版本的格子"上编辑，改的是**上一版**（往回看也会变）—— 右栏会写明是哪一版，
  想让它"从这一刻起才算"就先点「＋ 在这一格记一帧」。这是模式定义本身，不是 bug。
