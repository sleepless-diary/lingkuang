...
- 补回（`src/ui/detail.ts`）：标题旁新增一行「精度」下拉 + 按档位出现的部件框
  （选到「日」才出现 月/日，选到「分」才出现到 分；**精度=年时一个框都不出现，面板不变高**）。
  与打字共存 —— 打字改精度后重渲染会让下拉自动跟随。
  - 改细：缺的部件补默认值（月/日 → 1，时/分/秒 → 0）。不补的话落盘会写出「有日没月」的不完整时间。
  - 改粗：更细的部件**清成 `undefined`** —— `main.js:38-60` 的 `yearToDateStr` 是「有才写」
    （`hasMonth = n.month !== undefined && n.month !== null`），清掉才真的只写年份；
    否则详情面板又会把「312年」显示成「312年1月1日」（第二轮修过的老 bug）。
  - 部件框用 `Number.isFinite()` 判值：**时分秒的 0 是合法值**，`parseInt(...) || undefined` 会把 0 清掉。
  - 改部件后只刷新那行时间文字（不整块重渲染），否则在几个框之间 Tab 切换会被打断。
...
- 实测（真实 Electron + CDP）**15/15**：下拉默认「年」且不出现多余框 → 选「日」出现 月/日（默认 1/1）→
  填 7/15 后时间文字与数据同步、frontmatter 写成 `year: 312-07-15` → 选「分」出现四个框、填 9/30 写成
  `year: 312-07-15 09:30` → ★改回「年」后部件框消失、`month/day/hour/minute` 清空、
  **frontmatter 回到 `year: 312`** → 编辑器下拉选项为 年/月/日/时/分/秒。
