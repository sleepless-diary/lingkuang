...
| `rail-order.cjs` | 帧条（演变）**展开/收起的错峰顺序 15 项**（用户 2026-09-14：「展开面板时也有错分，**离已有帧节点越近的节点越先出现**；出场也是一样，**离已有帧越远的帧越先退场**，最后再**平滑切换最外层框的高度**」）：★1 逐行 14ms 错峰、★2 顺序**按距离升序**（近的先；种子两端都有版本 ⇒ 与 DOM 顺序不同）、★3 入场不透明度 `0 → 0.4`（**不许亮到 1**，否则会"先全亮再变虚"）、★4/★4b 框高动画带 `delay`、`fill:'both'`，点下去那一 tick 框还没长高、★5 演完框回自然高度、★6 收起先化幽灵、★7 退场顺序 = **距离降序**（远的先走）、★8 退场不透明度起点 `0.4`（不是 1）、★9 **裁切层停在旧高度**（否则退场中的行被裁没）而框的目标高度已经变矮、★10 演完收干净、★11 无异常。⚠️ 前置是**自己的**播种脚本 `seed-rail-order.cjs`；⚠️ 「距离」在断言里是**从 DOM 现算**的（行号差的最小值），不写死数字 |
...
# 起应用
node tools\e2e\rail-order.cjs                  # 15 项（A/B 改动前 7 项 FAIL）
```
