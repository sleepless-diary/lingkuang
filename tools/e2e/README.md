...
| `editor-tree.cjs` | 编辑器文件树 **28 项**（用户 2026-09-13：「**就是编辑器的树现在只能显示事件节点，其他结构体的文件夹没有在树里面**」）：★4 有节点的「事件」计数 2 → ★5/★6/★7/★9 **一个节点都没有的「战斗」也在树里**（计数 0 + `is-empty` + `opacity` 实测 **0.45** + 展开给一句「这个结构体还没有节点」）→ ★10/★11 时间线页签的树里有 **`_设定`** 分支（计数 = 实体数）→ ★13/★14/★15/★17 实体类型**全部列出**，空的「地点」同样置灰 + 一句「这个类型还没有实体」→ ★19 类型下有实体行 → ★22~★26 在**时间线页签**点实体行 = **切到「实体」页签**+标题/正文/字段/高亮都换成该实体（★24 防"节点正文写进实体文件"）→ ★27 无异常。⚠️ A/B：退回 HEAD 重新 build 只有 **7/28**。⚠️ 断言里用到的字段（如 `empty`）必须真的在 helper 返回的对象里 —— 本套件第一版就因漏读 `is-empty` 报过一次假 FAIL |
