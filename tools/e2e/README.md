...
| `assoc-canvas.cjs` | 灵感触发器·联想画布 **8 项**（用户 2026-09-12：「节点会被一块地方挡住，看不全 / 移出视窗时视窗不跟着移」）：★1 滚到底时**画布顶部不被 sticky 工具条压住**（`stageTop ≥ barBottom`、`stageBottom ≤ vh`、画布顶部那一圈 `elementFromPoint` 归画布、高度 ≥ vh×0.7）、★2 鼠标停在**画布上**滚滚轮真能滚页面（找的是真滚动容器 `.lk-tool-slot`，不是写死的类名）、★3 拖到右边缘**按住不放**→视窗自己往右推且节点**一直贴在鼠标下**、★3b 推到头夹在世界右墙（`panX = stageW − 2000`）、★4 松手后推力立刻停、★5 反向拖到左边缘 → 夹在世界左墙（`panX = 0`）、★6 无未捕获异常。⚠️ 自动推视窗是 rAF 驱动的 ⇒ 断言前必须 `forceFrames()`（见铁律 6） |
...
联想画布那条（**不依赖时间线数据**，哪个已播种的目录都能跑；它自己会 `assocSetRoot('雪原')` 造根节点）：

```powershell
# 起应用（同上面的 Start-Process）
node tools\e2e\assoc-canvas.cjs               # 8 项：遮挡 / 画布上滚轮 / 贴边自动推视窗
```

⚠️ 这条的「自动推视窗」是 rAF 驱动的，而测试实例是 hidden（铁律 6）⇒ 脚本里用 `forceFrames()`
