...
| `seed-editor-tree.cjs` | 前置：给「编辑器文件树」造确定起点 —— 节点结构体 `事件`（**有 2 个节点**）+ `战斗`（**一个节点都没有**，硬盘上也不建这个文件夹）；实体类型 `角色`（1 个实体）+ `地点`（**一个实体都没有**）。⚠️ 它整份写 `formats.json`（`main.js` 的 `loadFormatsRaw` 只要文件能解析就**直接用**、不跟内建合并）⇒ 空种类能不能出现在树里完全取决于这份文件。⚠️ 全新目录要**先起一次应用**写出 `worldbuilding.json` 再来播种 |
| `editor-tree.cjs` | 编辑器文件树 **34 项**（用户 2026-09-13：「**就是编辑器的树现在只能显示事件节点，其他结构体的文件夹没有在树里面**」+ 随后两条反馈「**点到设定里面的实体文件时测试世界观文件夹会消失，事件文件夹也没了**」「**设定文件夹和世界观文件夹处于同一缩进**」）：★4 有节点的「事件」计数 2 → ★5/★6/★7/★9 **一个节点都没有的「战斗」也在树里**（计数 0 + `is-empty` + `opacity` 实测 **0.45** + 展开给一句「这个结构体还没有节点」）→ ★10/★11 时间线页签的树里有 **`_设定`** 分支（计数 = 实体数）+ **★11b 缩进与「主线」一致（`paddingLeft` 都 18px，且 ≠ 世界行的 8px）** → ★13/★14/★15/★17 实体类型**全部列出**，空的「地点」同样置灰 + 一句「这个类型还没有实体」→ ★19 类型下有实体行 → ★22~★26b 在**时间线页签**点实体行 = **就地**换右栏（**★22 不切页签**、★23/★23b/★23c/★23d **世界/时间线/种类文件夹都还在、`_设定` 仍展开**、★23e 该实体行在**时间线那棵树里**高亮、★24~★26 标题/正文/字段都换成该实体、★26b 之后手动切「实体」页签打开的仍是它）→ ★27 无异常。⚠️ A/B：退回上一版（切页签 + `_设定` 无缩进）**27/34**，挂的正是 ★11b/★22/★23/★23b/★23c/★23d/★23e。⚠️ 断言里用到的字段（如 `empty`）必须真的在 helper 返回的对象里 —— 本套件第一版就因漏读 `is-empty` 报过一次假 FAIL |
...
**编辑器文件树那条**用独立目录（`lk-edtree`）—— 它要"有节点的种类"和"空种类"同时在树里：
...
node tools\e2e\seed-editor-tree.cjs          # 整份写 formats.json（事件 有节点 / 战斗 空）
# 起应用（同上面的 Start-Process，端口 9704）
node tools\e2e\editor-tree.cjs               # 28 项
```
