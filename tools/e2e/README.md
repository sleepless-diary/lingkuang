...
| `storyline-focus.cjs` | 剧情线**聚焦 = 线外截断 + 多段拼接**（第 4.0 片 C）**8 项**（用户原话：「剧情线之外的内容（包括时间线）全部截断，如果剧情线之间是多段时间则**连接两段时间**（剔除中间的节点和时间线）」）：★0 全览下 6 个节点都在、标尺无断口 → ★0b 对照：同一个算式（屏幕距离 ÷ 段内 px/年）在全览下算出**真实**的 3850 年 → ★1 聚焦两段线后**线外节点不出现**、线内 4 个都在 → **★2 两段直接相邻**：同一算式只剩 **50.2 年**（段间 3850 年被压掉）→ ★3 被截断区间 (200,4000) 里**一个刻度都没有** + 接缝有 `.tl__axis-cut` 断口 → ★4 段内刻度仍等距（只吃空隙、不动段内比例）→ ★5 切回「— 全览 —」完全还原 → ★6 无异常。⚠️ 前置 = `seed-storyline-focus.cjs`（可跑在全新目录上）；⚠️ 定标基准要够长（`left` 是整数像素，拿 50 年≈10px 当基准取整误差就有 10%，实测算出 3760）；⚠️ 视图是缓动的 ⇒ 读数要连续两次一致再断言；**★6/★7 是「非线性 × 聚焦一起开」**（用户 2026-09-19：「非线性和聚焦做一下适配，现在两个同时开有bug」）：非线性下只排线内 4 个节点、等距、时间遮罩/色带都被清掉；关掉非线性仍停在聚焦态（断口+色带回来）；**★8/★9 新建剧情线**（用户 2026-09-19：「在聚焦的下拉框底部增加一个新建剧情线的功能，点击在右侧面板展开新建剧情线的面板」）：下拉底部「＋ 新建剧情线…」⇒ 右侧展开创建面板（名字/段/创建/取消）且下拉**弹回**当前聚焦值；点「创建」⇒ 多一条线并聚焦它、右侧换成段面板、段的单位是**年**（不是 epoch 秒）。⚠️ **`stableView` 必须"连续 3 次一致 + 主刻度 ≥4 条"**：窗口不在前台时 rAF 被节流，缓动**中间态**的 DOM 会连续两次一模一样（实测抽样落在「可见范围整段落在被压掉的空隙里」那一帧 ⇒ ★3/★4/★7 假挂） |
| `seed-storyline-focus.cjs` | 前置：**两段式**剧情线 + 线内/线外都留节点（段 A = 100~200 年、段 B = 4000~4100 年；线内「甲段起点/甲段中点/乙段起点/乙段中点」，线外「空隙里的事件」2000、「末段之后」5000）。⚠️ 剧情线只存在 `worldbuilding.json` 的时间线里（vault 的 .md 承载不了）而节点以 .md 为源 ⇒ 两边都要写，且 JSON 里时间线的 key 必须是 `tl-主线`（`vaultToWorldData()` 按 `'tl-' + 名称` 生成 id） |
| `nonlinear-pan.cjs` | **非线性模式下画布可平移/缩放 7 项**（用户 2026-09-19：「非线性下节点固定在屏幕上了」）：★0 线性视图前置 → ★1 打开非线性：节点按**序列**等距（相邻间距一致）→ **★2 滚轮平移 → 所有节点同步右移 180px**（旧代码一动不动：x = `50 + i*pitch` 里没有 pan，那一支也不响应滚轮）→ ★3 Alt+滚轮缩放 → 间距 ×1.2 → ★4 双击＝回到「刚好铺满」（与刚进模式时逐项相同）→ ★5 关掉非线性回到线性（标尺回来）→ ★6 无异常。⚠️ 修法是给非线性**自己一套**视图游标 `nlPan/nlZoom`（不共用 `view.panX/spacing`）；⚠️ 夹具复用 `seed-storyline-focus.cjs`（6 个节点）；⚠️ **开跑前显式切回「— 全览 —」**（本套件量的 6 个节点＝全览下的全部节点，而聚焦态下非线性只排线内节点；`storyline-focus` 的收尾状态正是"聚焦中"，不重置会只看到 4 个） |
