...
# 起应用
node tools\e2e\codex-list-motion.cjs           # 20 项（A/B 改动前 19/20）
...
## ⚠️ 已知 fixture 漂移：`codex-tree-view.cjs` ★7/★8

该套件的 ★7/★8 断言「有实体的类型与**空类型**都在、空的置灰」。世界里 `entityTypes` 会被 vault 扫描结果
收窄成"硬盘上真有的那几个文件夹"，而 `reset-entity-vault.cjs` 会把 `_設定` 整个删掉、只留 `角色`
⇒ 这两条现在会挂（**20/22**）。**与源码改动无关**：`git stash` 退回 2026-09-14 那三个源文件 + `vite build`
后**同样 20/22、同样两条**。修法方向（本轮试过、被更大的连带影响劝退）：让 reset 既铺 `entityTypes`
又在硬盘上建五个类型空文件夹 —— 试的时候 `codex-node-tab` 的树层级断言一起挂（15/22），遂回退如实记档。
