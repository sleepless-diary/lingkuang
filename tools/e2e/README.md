...
| `seed-editor-tree.cjs` | 前置：给「编辑器文件树」造确定起点 —— **节点种类** `事件`（**有 2 个节点** ⇒ 该出现在时间线下）+ `战斗`（**一个节点都没有** ⇒ **不该**出现）+ `角色`（没有节点、且与实体类型同名 ⇒ **不该**出现，用户 2026-09-13 报的「角色/地点/物品等文件夹同时存在于主线与设定文件夹下」就是它）；**实体类型** `角色`（1 个实体）+ `地点`（**一个实体都没有** ⇒ 该出现，设定侧列全部类型）。⚠️ 它整份写 `formats.json`（`main.js` 的 `loadFormatsRaw` 只要文件能解析就**直接用**、不跟内建合并）⇒ 空种类能不能出现在树里完全取决于这份文件。⚠️ formats.json 是**启动时读**的 ⇒ 播完要重启应用；⚠️ 全新目录要**先起一次应用**写出 `worldbuilding.json` 再来播种 |
| `editor-tree.cjs` | 编辑器文件树 **33 项**（用户 2026-09-13：「**就是编辑器的树现在只能显示事件节点，其他结构体的文件夹没有在树里面**」+ 「**点到设定里面的实体文件时测试世界观文件夹会消失，事件文件夹也没了**」+「**设定文件夹和世界观文件夹处于同一缩进**」+「**我指的是角色，地点，物品等文件夹同时存在于主线与设定文件夹下，是bug**」）：★4 有节点的「事件」计数 2 → **★5/★5b/★5c 有定义但一个节点都没有的「战斗」「角色」不许出现在时间线下、时间线下的种类只有真有节点的那些**（★15b 全树里「角色」只出现一次且在 `_设定` 下）→ ★10/★11 时间线页签的树里有 **`_设定`** 分支（计数 = 实体数）+ **★11b 缩进与「主线」一致（`paddingLeft` 都 18px，且 ≠ 世界行的 8px）** → ★13/★14/★15/★17 实体类型**全部列出**，空的「地点」同样置灰 + 一句「这个类型还没有实体」→ ★19 类型下有实体行 → ★22~★26b 在**时间线页签**点实体行 = **就地**换右栏（**★22 不切页签**、★23/★23b/★23c/★23d **世界/时间线/种类文件夹都还在、`_设定` 仍展开**、★23e 该实体行在**时间线那棵树里**高亮、★24~★26 标题/正文/字段都换成该实体、★26b 之后手动切「实体」页签打开的仍是它）→ ★27 无异常。⚠️ A/B 三段：退回「只列已有节点的种类」那一版 **7/28**；退回「切页签 + `_设定` 无缩进」**27/34**；退回「列 formats 全部种类」**28/33**（挂 ★5/★5b/★5c/★15b/★23c，dump 里直接看到 `[事件, 战斗, 角色]` = 用户报的现象）。⚠️ 断言里用到的字段（如 `empty`）必须真的在 helper 返回的对象里 —— 本套件第一版就因漏读 `is-empty` 报过一次假 FAIL |
...
**编辑器文件树那条**用独立目录（`lk-edtree`）—— 它要"有节点的种类"和"空种类 / 与实体类型重名的种类"同时在 `formats.json` 里：
...
node tools\e2e\editor-tree.cjs               # 33 项
```
