...
| `codex-node-tab.cjs` | 设定库「时间线节点」页签 **17 项**：四级树 → 公共属性面板固定行 → **★9b 提交字段后面板不重建**（元素身份不变；★9c 那一次提交确实落进了 `.md`，防"什么都没发生"式的假绿）→ 改描述/正文落到节点的 `.md` → 搜索 → 跨页签不串文档。⚠️ ★9b/★9c 原来在 `editor-props-panel.cjs` 里，编辑器工具并进工作台后搬过来（用种类模板字段「地点」当探针，不动标题 —— 标题会改文件名，而本套件按固定路径读文件） |
| `codex-tree-view.cjs` | 工作台左栏**「文件夹树」形态 29 项**（用户 2026-09-13：「**设定库和编辑器是不是可以做成同一工具的两种不同形式啊（在设置里面切换）**」）：★1 左栏「列表 / 文件夹树」开关、开机形态来自设置且默认是列表 → ★3/★3b 切树后类型 chips 让位、改出世界层、搜索框提示改成「搜索条目…」 → ★5 **同一个世界下时间线与 `_设定` 分支同框**（这棵树原来只长在「编辑器」工具里）→ ★7/★9/★11 类型层列出全部类型（含一个实体都没有的、置灰 + 一句人话）→ ★13/★14 树里点实体行 = 就地换中栏/右栏（名字/字段/右栏帧条都在 + 该行高亮）→ ★16/★18/★20 时间线分支：种类只列真有节点的、点节点行换到节点且**左树没被换掉** → ★21 树视图里搜索同时命中节点与实体 → ★23/★24/★25 开关写进 `localStorage`（`workbenchView`）、设置面板那组单选跟着显示、**重开工作台直接是上次选的形态** → ★27 切回列表并复位（别污染同实例后续套件）。⚠️ `_设定` 行要**展开世界**之后才出现（树的展开态是各层自己的），所以 ★3 只断言"换成了树"，同框交给 ★5。⚠️ 自足：开头先把 `workbenchView` 复位成 `list` |
...
设定库工作台那几条（节点页签 + 两种左栏形态）跑一次：

```powershell
node tools\e2e\reset-entity-vault.cjs
node tools\e2e\seed-node.cjs                 # 播节点 + 种类模板 + 一条有正文的实体
# 起应用（同上面的 Start-Process）
node tools\e2e\codex-node-tab.cjs            # 17 项
node tools\e2e\codex-tree-view.cjs           # 29 项（文件夹树形态；结尾会把形态复位成列表）
