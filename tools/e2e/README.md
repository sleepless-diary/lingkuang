...
| `agent-panel.cjs` | 灵框助手**面板 19 项**（用户 2026-09-15：「ai 真的工作，类 agent…有一个自己的对话框…有记忆…还有一个全局快捷键」）：★0 前置（设定库开着）、★1 `Ctrl+K` 开出来的是 `position:fixed` / 贴右 / 宽 380 的面板、★2 **主区一动不动**（`#cx-root` 同一元素 + `#lk-module-view` 的 HTML 长度没变）、★3 头行与输入框都在、★4 「正在编」焦点 chip（`正在编：银发少女` / `正在编事件：王国的建立` / `最近在看：X` / `没打开条目`）与模型 chip（实测 `本地 · qwen2.5:7b`）、★5 **上下文打包**（`#lk-agent-ctx` 里有 `【工作区】当前世界「测试世界观」`/`【当前时间线】主线（1 个事件）`/`312 王国的建立·事件`/`【创作者此刻打开的那一条】设定「银发少女」（角色）` + **`文件：测试世界观/_设定/角色/银发少女.md`**，且**焦点块排在 `【设定】共` 之前**）+ 字段与正文）、★6 ⭐**换个条目焦点跟着换**（`lingkuang-agent-focus`；换条目是 UI 状态、不一定动数据，所以不能只靠 store 订阅）、★6b ⭐**切到别的工具焦点只降级不清空**（chip 变 `最近在看：王国的建立`、上下文里那条与它的文件路径仍在、末尾有「现在切到别的功能去了」、**不是** `NO_FOCUS`）、★6c **切回工作台 ⇒ 升回「正在编」**（降级标记消失）、★6d ⭐**世界沙盘上也报焦点**（用户 2026-09-18：「时间轴面板也要让它能看到我在哪个文件」）—— 在 `#lk-pane-timeline` 上点开一个事件 ⇒ chip 变 `正在编事件：王国的建立`、上下文里那条 + `文件：测试世界观/主线/事件/王国的建立.md` + `在哪：世界沙盘的时间线上`，降级标记消失；⚠️ **必须派 `PointerEvent`**：`pointerdown`（`button:0`、`buttons:1`、带 clientX/clientY/pointerId/pointerType）给节点元素 + `pointerup` 给 window，**不能派 pointermove**（会被当成拖动、不选中）；`el.click()` 一辈子选不中、★7 盘上的历史进对话框、★8 IPC 落盘 round-trip（磁盘上是**裸数组**，不是 `{chat:[…]}`）、★9 Esc 关、★10 `Ctrl+K` 再按＝关、★11 左栏「助手」按钮也能开关、★12 **面板只有一格**（开设置 ⇒ 助手让位；关掉后主区没动、助手还能再呼出 —— `registry.ts` 只有一格 `disposePanel`，同一时刻只能开一个面板）、★14 ⭐**助手看得到创作者过去的操作**（用户 2026-09-18：「能不能让这个 ai 能读到我的过去操作行为」）—— 改一个字段（`#cx-fields` 里「发色」那行的 `input` 设值 + 派 `change`）⇒ `#lk-agent-ctx` 里出现 `【他最近做过的事】`、`改了设定「银发少女」的字段「发色」` 与尾注「（这些是他自己动手改的；标着「灵框助手代劳」的才是你改的。）」、★15 同一条流水**自己落了盘**（`agent/activity.json` 里有含 `发色` 的 text）**且只带 activity 的那次 `agent:save` 没把对话历史抹掉**（`chat.json` 仍是种子那两条）—— `main.js` 的 `agent:save` 是「给了才写」（旧写法无条件写 chat，只传 activity 会把历史写成 `[]`）、⚠️ 这两条要**先轮询 `activity.json` 当同步点**（改字段前先 `unlinkSync`，否则上次的残留让轮询立刻通过；测试窗口 hidden ⇒ 定时器被节流，`BATCH_MS = 500` + `SAVE_MS = 1500` 的落盘晚于纸面值，死等固定毫秒会假挂）、★13 无未捕获异常。⚠️ 前置 = `reset-entity-vault.cjs` + `seed-node.cjs` + **`seed-agent-chat.cjs`**（同一 pwsh 调用里做，再重启实例）；⚠️ 走 `LK_AGENT_DIR` → `dirname(LINGKUANG_TEST_DATA)/agent` → `%TEMP%/lk-evault2/agent` 找历史文件 |
...
| `create-node-panel.cjs` | 新建节点面板 + 页签弹动 **13 项**（用户 2026-09-19：「给创建节点做专门适配（创建面板要有名字/年份/种类/模板字段 + 创建/取消，不要复用节点信息面板，现在连创建按钮都没有）；另外点创建节点时主线页签会上下弹动，修一下」）：★0 前置（沙盘 + `＋节点` + 主线页签计数 1）→ ★1/★1b **点「＋节点」开的是创建面板**（名字/年份/种类三栏 + 「创建」「取消」；**不是** `#d-view` 那套信息面板）且**不建节点** → ★2 面板里渲染出该种类的**模板字段**（`formats.json` 的 地点/规模，走 `fieldRow()`）→ ★3 「取消」什么都不建 → ★4~★4c 「创建」建出节点并关面板、**页签计数就地变 2 而页签没被重建**（同一元素、仍连着 DOM）→ **★5 点「创建」那一刻页签一动不动**（同一元素 + **同一 Animation 对象** + `top` 不变，700ms 后仍不变）→ ★6 模板字段值进了 `.md` 的 frontmatter（`地点: 北境`）→ ★7/★7b 页签**集合**变了照样重建、换页签照样播错峰（`lk-wake` 延迟 0/100ms）→ ★8 无异常。⚠️ 前置 = `reset-entity-vault.cjs` + `seed-node.cjs`（可跑在**全新目录**上）；⚠️ 两个断言坑写在文件头：CSS 动画 `fill:both` 跑完仍留在 `getAnimations()` 里（要比**对象身份**，不是"有没有动画"），且 `getAnimations()` 混着 hover 变色的 CSS transition（无 `animationName`，要筛掉） |
| `session-restore.cjs` | 会话恢复 **11 项**（用户 2026-09-19：「我希望进入灵框时默认是上次关闭时的状态」；范围＝上次的工具 / 世界+时间线 / 设定库正在编的那一条，实现见 `src/ui/session.ts`）：★0 没有存档时启动＝世界沙盘 → ★1/★2 打开设定库点中实体 ⇒ localStorage `lingkuang-session` 记下 tool/world/timeline/target → ★3 **重进后自己回到设定库并停在「银发少女」**（实体态）→ ★4/★5 换到时间线节点 ⇒ 存档改成节点身份（带 `tlId`）→ 重进后仍停在该节点（行高亮 + 中栏标题 + 正文都对）→ ★6 上次是灵感触发器 ⇒ 重进仍是它 → ★7 上次回到沙盘 ⇒ 重进仍是沙盘 → ★8 上次选的是新建的那条时间线 ⇒ 重进后选中态仍是它（**不是** order 第一条「主线」）→ ★9 存档写坏（往 localStorage 塞垃圾）⇒ 安静回默认、不炸 → ★10 无未捕获异常。⚠️ **「重进」用 `location.reload()` 模拟**：整份 `main()` 重跑一遍，跟重开应用同一条路，又不必起第二个 Electron；CDP 在 reload 期间**执行上下文会被换掉**（读会抛）⇒ 全程用带重试的 `evSoft`。⚠️ 前置 = `reset-entity-vault.cjs` + `seed-node.cjs`（可跑在全新目录上）；⚠️ 节点行按 `#cx-list [data-act="node"]` 找（`data-cx-id` 只挂实体行） |
