 ...
 *    window.__lkAgentMock = { chunks: ['一', '段', '应', '答'], gap: 60, truncated: true }
 *        // **分片**：一片一片喂给 onDelta（中间隔着 gap 毫秒的真定时器）—— 流式只有跨任务才看得见，
 *        //   同一个任务里把几片喂完，自动化那边读到的永远只有最后态。
 *
...
type MockReply = string | string[] | ((msgs: ChatMsg[]) => string | Promise<string>) | { chunks: unknown[]; gap?: number; truncated?: boolean };
...
export async function agentAsk(messages: ChatMsg[], opts?: AskOpts): Promise<AiReply> {
  ...
  if (mock !== undefined && mock !== null) {
    ...
    if (text && typeof text === 'object' && Array.isArray((text as { chunks?: unknown[] }).chunks)) {
      const m = text as { chunks: unknown[]; gap?: number; truncated?: boolean };
      ...
      return { text: parts.join(''), model: 'mock', truncated: m.truncated === true };
    }
